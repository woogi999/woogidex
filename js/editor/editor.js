...
import { AbilityList, LearnsetList } from '../react/EditorLists.jsx';
import { abilityRole, prepareLearnset } from './learnset-model.js';
...
// ==================== SHOWDOWN DATA ====================
        ...
        async function fetchShowdownData() {
            ...
            try {
                ...
                // upstream) so analysis.js can score off it instead of a hand-curated list
                ...
                for (const [key, m] of Object.entries(movesRaw)) {
                    ...
                    const move = { name: m.name || key, category: m.category || 'Status', type: m.type || 'Normal' };
...
// ==================== ADVANCED MOVE BROWSER ====================
        ...
        function openMoveBrowserModal() {
            // whoever opens the shared modal claims it; release any prior hold
            window.closeBattleMoveBrowser?.();
        ...
        function filterMoveBrowser() {
            // the battle teambuilder borrows this modal for its own learnset-scoped
            // list; defer to it while it owns the modal, or typing here would
            // replace its list with the whole move table
            if (window.battleMoveBrowserOwnsModal?.()) return window.filterBattleMoveBrowser();
...
// ==================== ABILITIES ====================
        ...
        // ./learnset-model.js, which is where that rule now lives.
        ...
        function describeAbility(a) {
            ...
            const sdEntry = Object.entries(state.sdAbilities).find(([, v]) => v.name === a.name);
            const desc = isCustom
        ...
        function renderAbilities() {
            mountIsland('abilities-list', AbilityList, {
                abilities: state.abilities,
                editingIndex: editingCustomAbilityIndex,
                describe: describeAbility
            });
            // Reordering by drag is part of the row component now; only the
            // click-away that closes an open editor is still global.
...
import { esc as escapeHtml, esc as escapeHtmlAttr } from '../core/html.js';
...
// ==================== LEARNSET ====================
        ...
        function renderLearnset() {
            ...
            // Entries keep their real index in state.learnset, so remove/update
            // handlers still target the right move after filtering and sorting.
            // See prepareLearnset() in ./learnset-model.js.
            const entries = prepareLearnset(state.learnset, {
                query: value('learnset-search').trim().toLowerCase(),
                typeFilter: value('learnset-filter-type'),
                catFilter: value('learnset-filter-category'),
                sortMode: document.getElementById('learnset-sort')?.value || 'default',
                order: document.getElementById('learnset-order')?.value || 'desc',
                isCustomMove
            });

            mountIsland('learnset-list', LearnsetList, {
                entries,
                // An empty learnset says nothing; an empty *filtered* one has to
                // explain itself, or it looks like the moves were lost.
                filtered: state.learnset.length > 0,
                isCustomMove,
...
// ==================== CUSTOM ABILITY LIBRARY ====================
        ...
        // and compiled by sd-hooks.js, the same bridge Showdown's own moves/
        // items/abilities go through -- Showdown hook syntax, nothing new to learn.

        // which of the three editors a raw-code box belongs to, so e.g. onBasePower
        // in a move isn't reported unsupported when it's the hook a move should use
        function rawCodeKind(idBase) {
            if (idBase === 'custom-move') return 'move';
            if (idBase === 'custom-item') return 'item';
            return 'ability';
        }

        // auto-repairs a missing comma/stray semicolon on any save path, not just
        // blur, so code doesn't persist broken; only punctuation is rewritten --
        // a full module/TypeScript paste is otherwise left untouched
        function readEntityRawCode(idBase) {
            const raw = document.getElementById(`${idBase}-raw-code`)?.value ?? '';
            if (!raw.trim()) return raw;
            const result = checkRawHooks(raw, rawCodeKind(idBase));
            return result.autoFixed && result.fixedText ? result.fixedText : raw;
        }
        function fillEntityRawCode(idBase, value) {
            const el = document.getElementById(`${idBase}-raw-code`);
            if (el) el.value = value || '';
            renderRawCodeHookList(idBase);
            checkEntityRawCode(idBase);
            // open the drawer when there's something in it, so saved code
            // is never invisible behind a collapsed summary
            const details = el?.closest('details');
            if (details) details.open = !!(value || '').trim();
        }

        // reports a syntax error while typing rather than mid-battle, and names
        // any unsupported hook. `applyFix` rewrites the field for mechanically
        // repairable mistakes, only on blur/save (never every keystroke)
        function checkEntityRawCode(idBase, applyFix = false) {
            const out = document.getElementById(`${idBase}-raw-code-status`);
            if (!out) return;
            const kind = rawCodeKind(idBase);
            const source = readEntityRawCode(idBase);
            if (!source.trim()) { out.textContent = ''; out.className = 'entity-raw-code-status'; return; }
            let result = checkRawHooks(source, kind);
            let repaired = '';
            if (result.autoFixed && result.fixedText && applyFix) {
                const field = document.getElementById(`${idBase}-raw-code`);
                if (field) field.value = result.fixedText;
                result = checkRawHooks(result.fixedText, kind);
                repaired = 'Fixed a missing comma/semicolon for you. ';
            } else if (result.autoFixed && result.fixedText) {
                repaired = 'A small punctuation fix will be applied when you save. ';
            }
            if (!result.ok) {
                out.textContent = result.error;
                out.className = 'entity-raw-code-status is-bad';
                return;
            }
            out.textContent = repaired + describeRawCodeResult(result, kind);
            out.className = `entity-raw-code-status ${rawCodeTone(result)}`;
        }

        // one sentence about what the box actually produced: what bound, what
        // was recognised as data, and what has nowhere to go
        ...
        // rendered from the bridge's own table, so it can't fall out of step
        function renderRawCodeHookList(idBase) {
            const host = document.getElementById(`${idBase}-raw-code-hooks`);
            if (!host || host.dataset.filled) return;
            host.innerHTML = supportedHooks(rawCodeKind(idBase))
                .map(h => `<code>${h}</code>`).join(' ');
...
function saveCustomItemLibraryEntry() {
    ...
    const rawCode = readEntityRawCode('custom-item');
...
// ==================== CUSTOM MOVES ====================
        ...
        log.debug('CUSTOM MOVE', 'Opening custom move modal', { index });
            ...
            if (index !== undefined && state.learnset[index] && isCustomMove(state.learnset[index])) {
                ...
                fillEntityRawCode('custom-move', m.rawCode);
