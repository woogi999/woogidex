import { calcStat as calcBattleStat } from '../battle/engine/dex.ts';
import { log } from '../core/log.ts';
import { state, api } from '../core/app.ts';

import { NATURE_DATA, NATURES, STAT_NAMES, findNatureByBoosts } from '../core/data.ts';
import { getSdMoveByName, loadCompetitiveMoveUsefulness } from './editor.ts';
import { notify, notifySync } from '../app/store.ts';
import { closeDialog, openDialog } from '../app/dialogs.tsx';
import { updatePreview } from './editor-core.ts';
import { form } from './draft.ts';
// ==================== sample sets ====================
        // competitive sample-set generator. intentionally local/deterministic --
        // never calls an external API, only the fakemon's learnset plus Showdown
        // move metadata already loaded into state.sdMoves.
        //
        // scoring tables are kept in one place so the generator can be tuned
        ...
        const SAMPLE_SET_CONFIG = {
            roleMeta: {
                ...
                wallbreaker: { name: 'Wallbreaker', attack: null as any, natureFast: null as any, natureSlow: null as any },
                bulkyAttacker: { name: 'Bulky Attacker', attack: null as any, natureFast: null as any, natureSlow: null as any },
                defensive: { name: 'Defensive', attack: null as any, natureFast: null as any, natureSlow: null as any },
                support: { name: 'Support', attack: null as any, natureFast: null as any, natureSlow: null as any },
                pivot: { name: 'Pivot', attack: null as any, natureFast: null as any, natureSlow: null as any },
                setupSweeper: { name: 'Setup Sweeper', attack: null as any, natureFast: null as any, natureSlow: null as any },
                hazard: { name: 'Hazard Setter/Remover', attack: null as any, natureFast: null as any, natureSlow: null as any },
                screens: { name: 'Screens', attack: null as any, natureFast: 'Timid', natureSlow: 'Bold' }
            },
            move: {
                stab: 34,
        ...
        // setup moves that also boost speed, so they outrank speed-less equivalents
        // (calm mind/nasty plot) when the Fakemon benefits from more speed.
        const SAMPLE_SET_SETUP_SPEED_BOOST = new Set(['Quiver Dance','Shell Smash','Shift Gear','Dragon Dance','Victory Dance']);

        // moves excluded from auto-selection despite being legal (manual selection
        // still allowed); genuinely useful moves are judged by sampleMoveIntrinsicScore instead.
        ...
        // low-value status/utility moves never used just to fill the fourth slot
        ...
        // damage moves that double as traps; only usable with an explicit trapping gameplan
        ...
        // moves whose value depends on a battle condition (weather/terrain); only
        // treated as coverage when the set supplies the condition itself or via ability.
        // aurora veil is utility rather than coverage but is conditional the same way.
        ...
        // legal/useful moves (e.g. Rest) that the auto-generator should never pick;
        // kept separate from SAMPLE_SET_BANNED_MOVES, which also gates manual-move legality
        const SAMPLE_SET_AUTOGEN_BANNED_MOVES = new Set(['Rest']);
        // fixed-cost self-damage (separate from scaling recoil) -- especially bad on
        // sets that want to set up and stick around, since half their HP is gone before they attack
        ...
        // attacks that survive usefulness filtering despite low sample-set frequency;
        // still gated by role/category/typing/movepool fit, not blanket auto-picks
        ...
        // caches pure scoring checks the beam search repeats across roles
        let sampleGenerationMemo: any = null;
        ...
        function getSampleSetProfile() {
            const stats = {
                hp: parseInt(String(form.stats.hp)) || 60,
                atk: parseInt(String(form.stats.atk)) || 60,
                def: parseInt(String(form.stats.def)) || 60,
                spa: parseInt(String(form.stats.spa)) || 60,
                spd: parseInt(String(form.stats.spd)) || 60,
                spe: parseInt(String(form.stats.spe)) || 60
            };
            const types = [
                form.type1,
                form.type2
        ...
        function sampleCoverageScore(move, types, chosen: any[] = []) {
        ...
        function sampleAbilityFitScore(ability, profile, role, chosenMoves: any[] = []) {
        ...
        function sampleMoveIsActuallyUseful(move, profile: any = null, role: any = null, set: any = null) {
        ...
        function sampleHasTrappingGameplan(profile, chosen, role = '') {
            const chosenNames = new Set<string>((chosen || []).map(m => m.name));
        ...
        function sampleHasExternalCondition(profile, chosen, condition) {
            ...
            if (condition === 'terrain') {
                return [...chosenNames].some(n => SAMPLE_SET_TERRAIN_SETTERS.has(n)) ||
        ...
        function sampleTeraBlastAllowed(profile, role, chosen: any[] = []) {
        ...
        // roles whose identity depends on a specific move kind (hazard/pivot). the
        // beam search can prune a branch carrying the required move before slot 4 if
        // non-required branches outscore it early (sampleSetPartialViability only
        // rejects missing-kind branches once chosen.length >= 3 -- too late).
        const SAMPLE_SET_ROLE_REQUIRED_KIND = { hazard: 'hazard', pivot: 'pivot' };

        function samplePickMoves(profile, role, seed, options: Record<string, any> = {}) {
            ...
            for (let slot = 0; slot < 4; slot++) {
                ...
                // roles like 'pivot'/'hazard' need their defining move kind to survive
                // the beam even if outscored early by unrelated branches, or
                // samplePickMoves would wrongly return [] -- guarantee one surviving
                // branch with a path to the required kind every slot.
        ...
        function sampleChooseNatureEVs(profile, role, chosenMoves: any[] = []) {
            const s = profile.stats;
        ...
        function sampleChooseAbility(profile, role, chosenMoves: any[] = []) {
        ...
        function sampleItemFitScore(item, profile, role, moves, ability = '') {
            ...
            if (item === 'Assault Vest') {
                // hard constraint, not a preference: never let AV win the item slot for
                // a set that needs a status move (Toxic, Roost, hazards, etc.) and
                // silently strip it. generateAssaultVestSampleSet builds the real AV
                // set separately with "no status" applied from the first move pick.
        ...
        // roles AV actually fits (all-out attacker that shrugs off a special hit);
        // setup/defensive/support/hazard/screens all lean on status moves instead
        const SAMPLE_SET_ASSAULT_VEST_ROLES = ['wallbreaker', 'bulkyAttacker', 'physicalSweeper', 'specialSweeper', 'pivot'];

        // builds a from-scratch AV set with noStatus applied from the first move
        // pick; returns null if no AV-eligible role works out for this Fakemon.
        function generateAssaultVestSampleSet(profile, roleScores, seed) {
            ...
            for (const role of eligible) {
                ...
                // pivot's required move could only exist as a status move for some
                // learnsets even with noStatus set - reject rather than publish a
        ...
        function sampleChooseItem(profile, role, chosenMoves, chosenAbility = '') {
            ...
            // item selection never mutates the moveset; it fits the gameplan the
            // moves already chose. Assault Vest scores itself out (sampleItemFitScore)
            // whenever the moves include a status move, so it never strips one out.
            return candidates
                .map((item, index) => ({
                    item,
        ...
        function sampleSetIdeaSimilarity(a, b) {
            ...
            const dominant = evs => Object.entries<any>(evs).filter(([,v]) => (v || 0) >= 200).map(([k]) => k).sort().join(',');
        ...
        // caches the expensive set-generation result, keyed by the current Fakemon
        // inputs, so opening the modal never rebuilds the beam search on every click
        let sampleSetSuggestionCache = { key: '', suggestions: null as any };
        ...
        function getSampleSetSuggestionCacheKey() {
            ...
            return [
                form.name || '',
                Object.values<any>(profile.stats).join(','),
                profile.types.join('/'),
        ...
        function generateSuggestedSampleSets(force = false) {
            ...
            const debugProfile = debugSample ? {
                name: (profile as Record<string, any>).name,
                stats: { ...(profile.stats || {}) },
                types: [...(profile.types || [])],
                abilities: [...(profile.abilities || [])],
                moves: (profile.moves || []).map(m => ({ name: m.name, type: m.type, category: m.category, basePower: m.basePower }))
            } : null;
            const debugTrace: Record<string, any> | null = debugSample ? { startedAt:new Date().toISOString(), cacheKey, profile:debugProfile, roleScores:{ ...roleScores }, roles:{} as Record<string, any>, suggestions:[] as any[] } : null;
            ...
            const seedSource = [form.name || '', Object.values<any>(profile.stats).join(','), profile.types.join('/'), profile.abilities.join('/'), profile.moves.map(m => `${m.name}:${m.type}:${m.category}:${m.basePower}`).join('|')].join('::');
        ...
        // ---- adding a set: the dialog is js/app/dialogs/sampleSets.tsx ----
        function openSampleSetModal() {
            log.debug('SAMPLE SETS', 'Opening sample set dialog');
            openDialog('sample-set-add', {});
        }

        function closeSampleSetModal() {
            closeDialog('sample-set-add');
        }

        /** The dialog's ideas, generated once the move data has loaded (it's cached per Fakemon). */
        async function suggestedSampleSets() {
            const loaded = state.sdMoveUsefulness && Object.keys(state.sdMoveUsefulness).length;
            if (!loaded) await loadCompetitiveMoveUsefulness().catch(() => {});
            try { return generateSuggestedSampleSets(); }
            catch (err: any) { log.error('SAMPLE SETS', 'Generator error', err); return []; }
        }
        ...
        function applySuggestedSampleSet(set) {
            if (!set) return;
        ...
        // after any change to a set: redraw now (the fields are controlled inputs), then the board and the save
        ...
        function updateSampleSet(index, field, value) {
            ...
            if (field.startsWith('evs.')) {
                set.evs[field.split('.')[1]] = parseInt(value) || 0;
            } else if (field.startsWith('ivs.')) {
                set.ivs[field.split('.')[1]] = Math.max(0, Math.min(31, parseInt(value) || 0));
            } else if (field.startsWith('moves.')) {
                set.moves[parseInt(field.split('.')[1])] = value;
            } else if (field === 'level') {
                set.level = Math.max(1, Math.min(100, parseInt(value) || 100));
            } else {
                set[field] = value;
            }
            setsChanged();
        ...
        /** Typing in the item box: plain text, no longer tied to a custom item. */
        function updateSampleSetItem(setIndex, value) {
            const set = state.sampleSets[setIndex];
            if (!set) return;
            set.item = value;
            if (set.itemCustom && value !== (state.customItems || []).find(x => x.id === set.itemCustomId)?.name) {
                set.itemCustom = false;
                set.itemCustomId = null;
                set.itemDesc = '';
            }
            setsChanged();
        }

        /** An item picked from the suggestions, main-game or your own. */
        function pickSampleSetItem(setIndex, item) {
            const set = state.sampleSets[setIndex];
            if (!set || !item) return;
            set.item = item.name;
            set.itemCustom = !!item.__customItem;
            set.itemCustomId = item.__customItem ? item.id : null;
            set.itemDesc = item.__customItem ? (item.desc || '') : '';
            setsChanged();
        }

        /** Up to 8 items, main-game and your own, whose name contains the text. */
        function sampleItemSuggestions(value) {
            const q = String(value || '').trim().toLowerCase();
            if (!q) return [];
            const matches = Object.values<any>(state.sdItems).filter(it => it.name.toLowerCase().includes(q)).slice(0, 8);
            const customItems = (state.customItems || [])
                .filter(it => it?.name && it.name.toLowerCase().includes(q))
                .slice(0, 8)
                .map(it => ({ ...it, __customItem: true }));
            return [...matches.map(it => ({ ...it, __customItem: false })), ...customItems]
                .filter((it, idx, arr) => arr.findIndex(x => x.name.toLowerCase() === it.name.toLowerCase()) === idx)
                .slice(0, 8);
        }

        /** Up to 8 moves from this Fakemon's learnset whose name contains the text. */
        function sampleMoveSuggestions(value) {
            const q = String(value || '').trim().toLowerCase();
            if (!q) return [];
        ...
        /** The icon beside a set's item: a custom item's artwork, or Showdown's item sprite. */
        function sampleItemIcon(set) {
            if (!set?.item) return '';
            const custom = set.itemCustomId ? (state.customItems || []).find(x => x.id === set.itemCustomId) : null;
            if (custom?.artwork) return custom.artwork;
            return 'https://play.pokemonshowdown.com/sprites/itemicons/' + set.item.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9\-]/g, '') + '.png';
        }
        ...
        function copySampleSet(index) {
            const set = state.sampleSets[index];
            const fakemonName = form.name || 'Fakemon';
        ...
        function generateShowdownExport(name, set) {
            ...
            // Showdown omits this line entirely at the default of 100
...
// ==================== stat calculation ====================
        ...
        // adapter for the shared battle-engine stat formula (dex.ts), so the editor
        // shows the same numbers battle will use rather than a drifted local copy
        function calcStat(base, ev, iv, nature, statKey, level) {
            return calcBattleStat(statKey, base, iv, ev, level, nature);
        }
        ...
        let lastEvToast = 0;
        /** One stat's EVs: at most 252, and never past 508 in total. */
        function updateSampleSetEV(setIndex, statKey, value) {
            const set = state.sampleSets[setIndex];
            if (!set) return;
            const ev = parseInt(value) || 0;
            const otherEVs = Object.entries<any>(set.evs).filter(([k]) => k !== statKey).reduce((sum, [, v]) => sum + (v || 0), 0);
            const clampedEV = Math.max(0, Math.min(ev, 508 - otherEVs, 252));
            if (clampedEV < ev && Date.now() - lastEvToast > 2000) {
                api.showToast('Cannot exceed 508 total EVs!', 'error');
                lastEvToast = Date.now();
            }
        ...
        /** A set's six stats as the card shows them: base, EVs, IVs, the result, and the nature's effect. */
        function sampleSetStatRows(set) {
            const data = NATURE_DATA[set.nature];
            return ['hp', 'atk', 'def', 'spa', 'spd', 'spe'].map(key => {
                const base = parseInt(form.stats[key]) || 60;
                const ev = set.evs[key] || 0;
                const iv = set.ivs[key] ?? 31;
                const boosted = !!(data && data.up === key && data.up !== data.down);
                const reduced = !!(data && data.down === key && data.up !== data.down);
                return { key, base, ev, iv, total: calcStat(base, ev, iv, set.nature, key, set.level), boosted, reduced };
            });
        }

        // drawn by js/app/components/editor/SampleSets.tsx
        function renderSampleSets() { notify(); }
...
export {
    updateSampleSet, updateSampleSetItem, pickSampleSetItem, sampleItemSuggestions, sampleMoveSuggestions, sampleItemIcon, sampleSetStatRows,
    removeSampleSet, copySampleSet, copySampleSetText, generateShowdownExport, openSampleSetModal, closeSampleSetModal, suggestedSampleSets,
    addBlankSampleSet, addSampleSet, applySuggestedSampleSet, formatEVSpread, calcStat, getAllAbilities, updateSampleSetEV, guessEVSpread,
    setNatureBoost, renderSampleSets, sampleMoveIsActuallyUseful
};
