 ...
 *
 * @param roots the named top-level containers
 */
export function buildContainerRegistry(roots: Record<string, any[]>): Record<string, any[]> {
    ...
    for (const [id, list] of Object.entries<any>(roots || {})) walk(list, id);
    return registry;
}
...
export function blockCategory(action, { BLOCK_GROUPS = [], ACTIONS = {} }: { BLOCK_GROUPS?: any; ACTIONS?: any } = {}) {
 ...
 */
export function splitParamTemplate(template, paramDefs: any[] = []): {text: string, param: Record<string, any>|null}[] {
    const parts = String(template ?? '').split('__');
    let next = 0;
    return parts.map((text, idx) => {
        if (idx === parts.length - 1) return { text, param: null as any };
 ...
 * @param boxes (null for a block not on screen; its curves are skipped)
 * @returns SVG path `d` attributes
 */
export function buildConnectorPaths(boxes: Array<{x:number,y:number,width:number,height:number} | null>): string[] {
    const paths: any[] = [];
