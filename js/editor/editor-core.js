...
import { POKEMON_TYPES, TYPE_EFFECTIVENESS } from '../core/data.js';
                ...
                <div class="bulk-row">
                    <div class="bulk-card${closestPhys?.isOwnFakemon ? ' bulk-card-own' : ''}">
                        ...
                        <div class="bulk-match">
                            <img class="bulk-match-sprite" src="${physSprite}" alt="${closestPhys ? closestPhys.name : ''}" loading="lazy" decoding="async" onerror="window.fallbackPokemonImage(this, '${String(closestPhys?.name || '').replace(/'/g, "\\'")}', '${String(closestPhys?.baseSpecies || '').replace(/'/g, "\\'")}')">
                    ...
                    <div class="bulk-card${closestSpec?.isOwnFakemon ? ' bulk-card-own' : ''}">
                        ...
                        <div class="bulk-match">
                            <img class="bulk-match-sprite" src="${specSprite}" alt="${closestSpec ? closestSpec.name : ''}" loading="lazy" decoding="async" onerror="window.fallbackPokemonImage(this, '${String(closestSpec?.name || '').replace(/'/g, "\\'")}', '${String(closestSpec?.baseSpecies || '').replace(/'/g, "\\'")}')">
...
function setPreviewArtworkMode(mode) {
    ...
    // scope to the board container rather than a bare getElementById, in case
    // a stray element with the same id exists elsewhere in the DOM
    ...
    if (!image) {
        // target node stale/missing - force a full re-render instead of no-op
