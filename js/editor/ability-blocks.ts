...
import { parseRawEntry } from '../battle/engine/sd-hooks.ts';
...
// ==================== small helpers ====================
import { esc as escapeHtml } from '../core/html.ts';
...
import { buildContainerRegistry, ensureBlockPosition } from './ability-board-model.ts';
import { closeDialog } from '../app/dialogs.tsx';

const TRIGGERS = {
    switchIn: {
        label: 'This Pokémon switches in', icon: 'log-in', kinds: ['ability', 'item'], params: [] as any[],
        ...
        sd: () => ({ header: 'onStart(pokemon) {', footer: '}', selfVar: 'pokemon', foeVar: 'foe', moveVar: null as any, preamble: 'const foe = pokemon.side.foe.active[0];' }),
        es: () => ({ adder: 'Battle::AbilityEffects::OnSwitchIn', args: 'ability, battler', selfVar: 'battler', foeVar: 'foe', moveVar: null as any, preamble: 'foe = battler.pbDirectOpposing rescue nil', footer: null as any })
    },
    statModify: {
        ...
        es: (ast) => ({ adder: 'Battle::AbilityEffects::StatCalcFromAbility', args: 'ability, battler, stat, stageMul, stageDiv, value', selfVar: 'battler', foeVar: 'nil', moveVar: null as any, preamble: `next value if stat != :${STAT_ESSENTIALS[ast.triggerParams.stat || 'atk']}`, footer: 'next value' })
    },
    damagingHit: {
        label: 'This Pokémon is hit by a damaging move', icon: 'sword', kinds: ['ability', 'item'], params: [] as any[],
        ...
        es: () => ({ adder: 'Battle::AbilityEffects::AfterMoveUseFromTarget', args: 'ability, user, target, move, switchedBattlers, hpLost', selfVar: 'target', foeVar: 'user', moveVar: 'move', preamble: '', footer: null as any })
    },
    residual: {
        label: 'The end of each turn', icon: 'clock', kinds: ['ability', 'item'], params: [] as any[],
        ...
        sd: () => ({ header: 'onResidual(pokemon) {', footer: '}', selfVar: 'pokemon', foeVar: 'foe', moveVar: null as any, preamble: 'const foe = pokemon.side.foe.active[0];' }),
        es: () => ({ adder: 'Battle::AbilityEffects::EndOfRoundEffect', args: 'ability, battler', selfVar: 'battler', foeVar: 'foe', moveVar: null as any, preamble: 'foe = battler.pbDirectOpposing rescue nil', footer: null as any })
    },
    moveImmunity: {
    ...
    switchOut: {
        label: 'This Pokémon switches out', icon: 'log-out', kinds: ['ability', 'item'], params: [] as any[],
        allowed: ['healDamage', 'cureStatus', 'boostStat', 'showMessage', 'restoreItem'],
        sd: () => ({ header: 'onSwitchOut(pokemon) {', footer: '}', selfVar: 'pokemon', foeVar: 'foe', moveVar: null as any, preamble: 'const foe = pokemon.side.foe.active[0];' }),
        es: () => ({ adder: 'Battle::AbilityEffects::OnSwitchOut', args: 'ability, battler, endOfBattle', selfVar: 'battler', foeVar: 'nil', moveVar: null as any, preamble: '', footer: null as any })
    },
    turnStart: {
        label: 'The start of each turn', icon: 'play', kinds: ['ability', 'item'], params: [] as any[],
        ...
        sd: () => ({ header: 'onBeforeTurn(pokemon) {', footer: '}', selfVar: 'pokemon', foeVar: 'foe', moveVar: null as any, preamble: 'const foe = pokemon.side.foe.active[0];' }),
        es: () => ({ adder: 'Battle::AbilityEffects::StartOfRoundEffect', args: 'ability, battler', selfVar: 'battler', foeVar: 'nil', moveVar: null as any, preamble: '', footer: null as any })
    },
    beforeMove: {
        label: 'Just before this Pokémon moves', icon: 'chevrons-right', kinds: ['ability', 'item'], params: [] as any[],
        allowed: ['boostStat', 'healDamage', 'setStatus', 'cureStatus', 'changeType', 'addVolatile', 'removeVolatile', 'showMessage'],
        sd: () => ({ header: 'onBeforeMove(pokemon, target, move) {', footer: '}', selfVar: 'pokemon', foeVar: 'target', moveVar: 'move', preamble: '' }),
        es: () => ({ adder: 'Battle::AbilityEffects::OnStartUsingMove', args: 'ability, user, move, battle', selfVar: 'user', foeVar: 'nil', moveVar: 'move', preamble: '', footer: null as any })
    },
    afterMove: {
        label: 'Just after this Pokémon moves', icon: 'chevrons-left', kinds: ['ability', 'item'], params: [] as any[],
        ...
        es: () => ({ adder: 'Battle::AbilityEffects::OnEndUsingMove', args: 'ability, user, targets, move, battle', selfVar: 'user', foeVar: 'nil', moveVar: 'move', preamble: '', footer: null as any })
    },
    onContact: {
        label: 'This Pokémon is hit by a contact move', icon: 'hand', kinds: ['ability', 'item'], params: [] as any[],
        ...
        es: () => ({ adder: 'Battle::AbilityEffects::AfterMoveUseFromTarget', args: 'ability, user, target, move, switchedBattlers, hpLost', selfVar: 'target', foeVar: 'user', moveVar: 'move', preamble: 'next if !move.contactMove?', footer: null as any })
    },
    onFaint: {
        label: 'This Pokémon faints', icon: 'skull', kinds: ['ability', 'item'], params: [] as any[],
        allowed: ['dealDamage', 'setWeather', 'setTerrain', 'boostStat', 'showMessage'],
        sd: () => ({ header: 'onFaint(pokemon) {', footer: '}', selfVar: 'pokemon', foeVar: 'foe', moveVar: null as any, preamble: 'const foe = pokemon.side.foe.active[0];' }),
        es: () => ({ adder: 'Battle::AbilityEffects::OnBeingHit', args: 'ability, user, target, move, battle', selfVar: 'target', foeVar: 'user', moveVar: 'move', preamble: 'next if !target.fainted?', footer: null as any })
    },
    onStatus: {
        label: 'This Pokémon is given a status', icon: 'alert-circle', kinds: ['ability', 'item'], params: [] as any[],
        allowed: ['cureStatus', 'boostStat', 'healDamage', 'showMessage'],
        sd: () => ({ header: 'onAfterSetStatus(status, target, source, effect) {', footer: '}', selfVar: 'target', foeVar: 'source', moveVar: null as any, preamble: '' }),
        es: () => ({ adder: 'Battle::AbilityEffects::OnStatusInflicted', args: 'ability, battler, user, status', selfVar: 'battler', foeVar: 'user', moveVar: null as any, preamble: '', footer: null as any })
    },
    moveUsed: {
        ...
        es: () => ({ adder: 'Battle::MoveEffects::AfterMoveUse', args: 'move, user, targets, battle', selfVar: 'user', foeVar: 'targets[0]', moveVar: 'move', preamble: '', footer: null as any })
    },
    moveHit: {
        ...
        es: () => ({ adder: 'Battle::MoveEffects::OnHit', args: 'move, user, target, battle', selfVar: 'user', foeVar: 'target', moveVar: 'move', preamble: '', footer: null as any })
    },
    accuracyModify: {
        ...
        es: (ast) => (ast.triggerParams.role || 'user') === 'user'
            ? { adder: 'Battle::AbilityEffects::AccuracyCalcFromUser', args: 'ability, mods, user, target, move, type', selfVar: 'user', foeVar: 'target', moveVar: 'move', preamble: '', footer: null as any }
            : { adder: 'Battle::AbilityEffects::AccuracyCalcFromTarget', args: 'ability, mods, user, target, move, type', selfVar: 'target', foeVar: 'user', moveVar: 'move', preamble: '', footer: null as any }
    },
    statStageIgnore: {
        ...
        sd: (ast) => {
            ...
            return {
                ...
                footer: '}', selfVar: 'holder', foeVar: 'pokemon', moveVar: null as any, preamble: ''
            };
        },
        es: (ast) => ({
    ...
    moveIncoming: {
        label: 'A move is about to hit this Pok\u00e9mon', icon: 'shield-alert', kinds: ['ability', 'item'], params: [] as any[],
    ...
    damageModify: {
        ...
        es: (ast) => (ast.triggerParams.direction || 'dealing') === 'dealing'
            ? { adder: 'Battle::AbilityEffects::DamageCalcFromUser', args: 'ability, user, target, move, mults, power, type', selfVar: 'user', foeVar: 'target', moveVar: 'move', preamble: '', footer: null as any }
            : { adder: 'Battle::AbilityEffects::DamageCalcFromTarget', args: 'ability, user, target, move, mults, baseDmg, type', selfVar: 'target', foeVar: 'user', moveVar: 'move', preamble: '', footer: null as any }
    }
};
...
for (const [alias, target] of Object.entries<any>(TRIGGER_ALIASES)) {
...
const CONDITIONS = {
    ...
    moveMakesContact: {
        label: 'move has the contact flag', params: [] as any[],
...
const ACTIONS = {
    ...
    ignoreStatStages: {
        label: 'Ignore those stat stages', params: [] as any[],
    ...
    swapItems: {
        label: 'Swap held items between this Pokémon and the opposing one', params: [] as any[],
...
Object.assign(TRIGGERS, {
    battleStart: {
        label: 'On Battle Start', icon: 'play-circle', params: [] as any[], allowed: [] as any[],
        sd: () => ({ header: 'onStart(pokemon) {', footer: '}', selfVar: 'pokemon', foeVar: 'pokemon.side.foe.active[0]', moveVar: null as any, preamble: '' }),
        es: () => ({ adder: 'Battle::AbilityEffects::OnSwitchIn', args: 'ability, battler', selfVar: 'battler', foeVar: 'battler.pbDirectOpposing rescue nil', moveVar: null as any, preamble: '', footer: null as any })
    },
    switchInOut: {
        label: 'On Switch In / Out', icon: 'repeat-2', params: [{ key: 'direction', label: 'Event', type: 'select', options: [{ value: 'in', label: 'Switch In' }, { value: 'out', label: 'Switch Out' }], default: 'in' }], allowed: [] as any[],
        sd: ast => ast.triggerParams.direction === 'out'
            ? { header: 'onSwitchOut(pokemon) {', footer: '}', selfVar: 'pokemon', foeVar: 'pokemon.side.foe.active[0]', moveVar: null as any, preamble: '' }
            : { header: 'onStart(pokemon) {', footer: '}', selfVar: 'pokemon', foeVar: 'pokemon.side.foe.active[0]', moveVar: null as any, preamble: '' },
        es: () => ({ adder: 'Battle::AbilityEffects::OnSwitchIn', args: 'ability, battler', selfVar: 'battler', foeVar: 'battler.pbDirectOpposing rescue nil', moveVar: null as any, preamble: '', footer: null as any })
    },
    turnStartEnd: {
        label: 'On Turn Start / End', icon: 'clock-3', params: [{ key: 'phase', label: 'Phase', type: 'select', options: [{ value: 'start', label: 'Turn Start' }, { value: 'end', label: 'Turn End' }], default: 'start' }], allowed: [] as any[],
        sd: ast => ast.triggerParams.phase === 'start'
            ? { header: 'onTurnStart(source) {', footer: '}', selfVar: 'source', foeVar: 'source.side.foe.active[0]', moveVar: null as any, preamble: '' }
            : { header: 'onResidual(pokemon) {', footer: '}', selfVar: 'pokemon', foeVar: 'pokemon.side.foe.active[0]', moveVar: null as any, preamble: '' },
        es: () => ({ adder: 'Battle::AbilityEffects::EndOfRoundEffect', args: 'ability, battler', selfVar: 'battler', foeVar: 'battler.pbDirectOpposing rescue nil', moveVar: null as any, preamble: '', footer: null as any })
    },
    onMove: {
        label: 'On Move', icon: 'swords', params: [{ key: 'phase', label: 'Phase', type: 'select', options: [{ value: 'use', label: 'Move Used' }, { value: 'hit', label: 'Move Hits' }], default: 'use' }], allowed: [] as any[],
        sd: () => ({ header: 'onAfterMove(source, target, move) {', footer: '}', selfVar: 'source', foeVar: 'target', moveVar: 'move', preamble: '' }),
        es: () => ({ adder: 'Battle::AbilityEffects::AfterMoveUseFromUser', args: 'ability, user, target, move, switchedBattlers, hitNum, canSwitch', selfVar: 'user', foeVar: 'target', moveVar: 'move', preamble: '', footer: null as any })
    },
    onDamage: {
        label: 'On Damage', icon: 'heart-crack', params: [] as any[], allowed: [] as any[],
        sd: () => ({ header: 'onDamagingHit(damage, target, source, move) {', footer: '}', selfVar: 'target', foeVar: 'source', moveVar: 'move', preamble: '' }),
        es: () => ({ adder: 'Battle::AbilityEffects::AfterMoveUseFromTarget', args: 'ability, user, target, move, switchedBattlers, hpLost', selfVar: 'target', foeVar: 'user', moveVar: 'move', preamble: '', footer: null as any })
    }
});
...
Object.assign(CONDITIONS, {
    moveIsDamaging: {
        label: 'move is damaging', params: [] as any[],
    ...
    targetIsFainted: {
        label: 'target is fainted', params: [] as any[],
...
function genericCodeComment(label, params) {
    const bits = Object.entries<any>(params || {}).map(([k,v]) => `${k}=${v}`).join(', ');
...
// customization.
Object.values<any>(TRIGGERS).forEach(t => { t.allowed = Object.keys(ACTIONS); });
...
// ==================== AST helpers ====================
function makeActionBlock(type): any {
    const def = ACTIONS[type]; if (!def) return null;
    const params: Record<string, any> = {}; (def.params || []).forEach(p => params[p.key] = p.default);
    return { id: uid(), kind: 'action', action: type, params };
}
function makeIfBlock(): any {
    const firstKey = Object.keys(CONDITIONS)[0]; const def = CONDITIONS[firstKey];
    const condParams: Record<string, any> = {}; (def.params || []).forEach(p => condParams[p.key] = p.default);
    return { id: uid(), kind: 'if', condition: firstKey, condParams, then: [] as any[], else: [] as any[] };
}
function makeEventBlock(trigger): any {
    const def = TRIGGERS[trigger] || TRIGGERS.switchIn;
    const params: Record<string, any> = {}; (def.params || []).forEach(p => params[p.key] = p.default);
    return { id: uid(), kind: 'event', trigger: trigger || 'switchIn', triggerParams: params, body: [] as any[], pos: { x: 24, y: 24 } };
}
function normalizeAbilityBlocks(raw) {
    ...
    return { version: 3, triggers: [ev], loose: [] as any[] };
}
...
function compileShowdownAbility(ability) {
    ...
    events.forEach(ev => {
        ...
        if (!groups.has(key)) groups.set(key, { t, bodies: [] as any[] });
        groups.get(key).bodies.push(...body);
        if (t.preamble) groups.get(key).preamble = t.preamble;
    });
    const handlers = [...groups.values()].map(({t,bodies,preamble}: { t?: any; bodies?: any; preamble?: any }) => {
...
function hasExportableCode(lib: any): lib is Record<string, any> {
    if (!lib) return false;
...
// js/battle/engine/sd-hooks.ts); a mod file needs one canonical shape, so the
...
function showdownSourceFromRaw(rawCode, indent = '\t\t') {
    ...
    const lines: any[] = [];
    for (const [key, value] of Object.entries<any>(parsed.entry)) {
...
// ==================== editor UI state ====================
let abState: any = null;       // AST currently being edited
let abAbilityId: any = null;   // library entry id currently being edited
...
let abKind: 'ability' | 'move' | 'item' = 'ability';
let abContainerRegistry: Record<string, any> = {};
let abDragId: any = null;
let abDragGrabOffset: any = null; // where the cursor grabbed the currently-dragged block, relative to its top-left
let abPreviewTab: 'sd' | 'es' = 'sd';
let abPrevViewId: any = null;  // which top-level view ('editor-view' / 'collection-view') to restore on close
...
function openAbilityBlockEditor(customAbilityId, kind: 'ability' | 'move' | 'item' = 'ability') {
    ...
    ['custom-ability', 'custom-move', 'custom-item', 'custom-entity-chooser'].forEach(closeDialog);
    const editorView = document.getElementById('editor-view');
    abPrevViewId = editorView && editorView.style.display !== 'none' ? 'editor-view' : 'collection-view';
...
function onTopLevelNavigation() {
    ...
    ['custom-ability', 'custom-entity-chooser'].forEach(closeDialog);
    abState = null;
    abAbilityId = null;
    abKind = 'ability';
    abContainerRegistry = {};
...
// central activateTopLevelView() lifecycle in app.ts.
...
function setAbilityBlockPreviewTab(tab) {
    abPreviewTab = tab === 'es' ? 'es' : 'sd';
    notify();
}

// ==================== rendering ====================
// The page is js/app/pages/AbilityBlockEditorPage.tsx, drawn from
// abilityBlockView(); the board's layout rules are in ./ability-board-model.ts.
...
function startAbilityBlockMove(event, id, containerId) {
    ...
    try { el.setPointerCapture(event.pointerId); } catch (e: any) {}
...
function finishAbilityBlockMove(event) {
    ...
    const slot = under?.closest?.<HTMLElement>('.ab-connection-slot');
    ...
    try { el?.releasePointerCapture?.(event.pointerId); } catch (e: any) {}
...
function handlePaletteDragStart(event, type) {
    ...
    try { event.dataTransfer.setData('text/ability-block', type); event.dataTransfer.effectAllowed = 'copy'; } catch (e: any) {}
}
...
// what the board's parameter dropdowns and the palette are built from
const BOARD_TABLES = {
    TRIGGERS, ACTIONS, CONDITIONS, BLOCK_GROUPS, EVENT_GROUP,
...
function handlePaletteEventDragStart(event, trigger) {
    abDragGrabOffset = null;
    try { event.dataTransfer.setData('text/ability-event', trigger); event.dataTransfer.effectAllowed='copy'; } catch(e: any) {}
}
...
function collectDescendantContainerIdsForDrop(block, out: any[] = []) {
    if (!block) return out;
...
function moveBlockIntoContainer(blockId, containerId, beforeId: any = null) {
...
function getDraggedAbilityBlockId(event) {
    ...
    try { return event?.dataTransfer?.getData('text/ability-block-id') || ''; } catch (e: any) { return ''; }
}
function handleConnectionDragOver(event, containerId) {
    ...
    try { paletteType = event.dataTransfer.getData('text/ability-block'); paletteEvent = event.dataTransfer.getData('text/ability-event'); } catch (e: any) {}
...
function highlightConnectionTargets(block, active) {
    document.querySelectorAll<HTMLElement>('.ab-connection-slot').forEach(slot => {
        const cid = slot.dataset.containerId;
...
function updateAbilityEventParam(id, key, rawValue) {
    ...
    ev.triggerParams[key] = p?.type === 'number' ? Number(rawValue) : rawValue; renderAbilityBlockEditor(true);
}

/** The compiled preview of what's on the board, in both languages. */
function abilityBlockPreview() {
    ...
    let sd, es;
    try { sd = compileShowdownAbility(previewAbility) || '// Add some blocks to see generated code.'; }
    catch (e: any) { sd = '// Could not generate preview: ' + e.message; }
    try { es = compileEssentialsAbility(previewAbility) || '# Add some blocks to see generated code.'; }
    catch (e: any) { es = '# Could not generate preview: ' + e.message; }
    return { sd, es };
}

/**
 * Everything the block editor page shows, or null when nothing is open.
 */
function abilityBlockView(): null | { kind: 'ability'|'move'|'item', Noun: string, name: string, events: any[], loose: any[], tables: any, previewTab: 'sd'|'es', sd: string, es: string } {
    if (!abState) return null;
    const info = abKindInfo();
    return {
        kind: abKind, Noun: info.Noun, name: abEntry()?.name || '',
        events: getAbilityRootEvents(), loose: getAbilityLooseBlocks(), tables: BOARD_TABLES,
        previewTab: abPreviewTab, ...abilityBlockPreview()
    };
}

/**
 * Redraws the page after the AST changed. sync: for an edit made from a
 * controlled field (a dropdown), which React would otherwise put back.
 */
function renderAbilityBlockEditor(sync = false) {
    if (!abState) return;
    ...
    if (sync) notifySync(); else notify();
}

export {
    abilityBlockView, updateAbilityEventParam,
    handleConnectionDragOver, handleConnectionDragLeave, handleConnectionDrop,
    handleBlockConnectionDragStart, handleBlockConnectionDragEnd, addAbilityEvent,
