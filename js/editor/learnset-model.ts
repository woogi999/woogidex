 ...
 *
 * @returns '' for an ordinary slot
 */
export function abilityRole(index, count): 'Hidden'|'Event'|'' {
 ...
 */
export function prepareLearnset(learnset: any[], {
    query = '', typeFilter = '', catFilter = '',
    sortMode = 'default', order = 'asc',
    isCustomMove = () => false
}: { query?: any; typeFilter?: any; catFilter?: any; sortMode?: any; order?: any; isCustomMove?: any } = {}): {m: Record<string, any>, i: number}[] {
    let entries = (learnset || []).map((m, i) => ({ m, i }));

    entries = entries.filter(({ m }: { m?: any }) => matchesQuery(m, query));
    if (typeFilter) entries = entries.filter(({ m }: { m?: any }) => (m.type || '') === typeFilter);
    if (catFilter) entries = entries.filter(({ m }: { m?: any }) => (m.category || '') === catFilter);
...
/**
 * The flags the custom move editor offers, in its order. reflectable and
 * snatch only apply to status moves (a status move is what gets bounced or
 * snatched), so the editor hides them otherwise.
 */
export const MOVE_FLAG_OPTIONS = [
    ['contact', 'Contact'], ['punch', 'Punch'], ['slicing', 'Slicing'], ['sound', 'Sound'], ['bite', 'Bite'],
    ['bullet', 'Bullet'], ['pulse', 'Pulse'], ['wind', 'Wind'], ['dance', 'Dance'], ['powder', 'Powder'],
    ['heal', 'Heal'], ['thawing', 'Thaws User'], ['charge', 'Charge'], ['recharge', 'Recharge'], ['highcrit', 'High Crit'],
    ['multihit', 'Multi-hit'], ['pivot', 'Pivot'], ['protect', 'Bypass Protect'], ['reflectable', 'Bypass Reflect'],
    ['snatch', 'Bypass Snatch'], ['bypasssub', 'Bypass Substitute']
];
