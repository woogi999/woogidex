...
import { sampleMoveIsActuallyUseful } from '../editor/sample-sets.ts';
import { POKEMON_TYPES, TYPE_EFFECTIVENESS, STAT_NAMES } from '../core/data.ts';
...
let usagePromise: Record<string, any> = {};
let tierDataCache: Record<string, any> = {};
let tierDataPromise: Record<string, any> = {};
let analysisTimer: any = null;
let analysisBusy = false;
let lastAnalysisKey = '';

/** A matchup card's sprite, and the image to fall back to when the animated one is missing. */
function matchupSprite(p){
  const id=String(p?.id||p?.name||'missingno').toLowerCase().replace(/[^a-z0-9-]+/g,'-');
  const fallback=`https://play.pokemonshowdown.com/sprites/gen5/${id}.png`;
  if(typeof api.getPokemonTemplateSprite==='function') return {src:api.getPokemonTemplateSprite(p),fallback};
  const spriteDir=api.getUse2DSprites?.() ? 'gen5ani' : 'ani';
  return {src:`https://play.pokemonshowdown.com/sprites/${spriteDir}/${id}.gif`,fallback};
}
...
// TYPE_EFFECTIVENESS keys are exact-cased; external data isn't guaranteed to match,
// and a bad lookup fails silently (undefined -> treated as neutral). Normalize once.
const TYPE_KEY_LOOKUP=new Map<any, any>(Object.keys(TYPE_EFFECTIVENESS).map(k=>[k.trim().toLowerCase(),k]));
function typeEffectivenessOf(attackType,defendType){
  ...
  if(!defKey) return 1;
  // Missing entry in a valid row = neutral (1x) by chart convention.
  const v=row[defKey];
...
function abilityNames(raw){
  ...
  if(raw&&typeof raw==='object')return Object.values<any>(raw).map(x=>typeof x==='string'?x:x?.name).filter(Boolean);
  return raw?[String(raw)]:[];
}
// Scored off Showdown's own ability ratings (state.sdAbilities) rather than a
// hand-curated list, so it stays current as Showdown's opinions change.
// Showdown rates -1 (detrimental) to 5 (format-defining), folded onto our -1/0/1/2/3
// scale: 5=3 (format-defining, e.g. Wonder Guard/Arena Trap), 4+=2 (elite), 3+=1 (strong),
// -1=drags score down, everything else (incl. unknown/custom) = neutral.
import { toId as toShowdownId } from '../core/html.ts';
...
function abilityQuality(names){
  // Best-case ability chosen (a competitive build picks its best ability).
  // Returns -1/0/1/2/3 plus the name responsible; higher tiers always outrank lower ones.
  // A weak ability only penalizes when it's the mon's *only* option - otherwise a
  // real teambuilder just runs the other ability instead.
  let best={score:0,name:null as any};
  let weak: any=null;
  let allWeak=true;
...
function uniqueDex(){
  const seen=new Set();
  return Object.values<any>(state.sdPokedex||{}).filter(p=>{
    if(!p?.name||!p?.stats||!p?.types||!p.num||p.num<0)return false;
    // Exclude Mega/Primal/Gmax: battle-only forms with inflated stats (nearly all
    // National Dex Uber) that skewed ordinary fakemon toward an Ubers verdict.
...
function moveIndex(){
  if(!state.__analysisMoveIndex){
    const idx=new Map();
    for(const [k,m] of Object.entries<any>(state.sdMoves||{})){
...
function learnsetRecord(p){
  // Many cosmetic formes (Ogerpon masks, Rotom forms) share the base species'
  // learnset under the base species' normalized id, not their own entry.
  const candidates=[
    p?.id,
    normalizeName(p?.id),
...
function getTarget(){
  const stats: Record<string, any>={};STAT_KEYS.forEach(k=>stats[k]=Number(form.stats[k])||0);
  return {
    id:'woogidex-current',
    name:form.name||'Current Fakemon',
    types:[form.type1,form.type2].filter(Boolean),
    stats,abilities:state.abilities||[],learnset:state.learnset||[],
    // Always the user's in-progress entry, never a real dex lookup - without this
    // flag a name collision with a real Pokemon would silently pull its learnset.
    fake:true
  };
}
function fakeDex(f){
...
function parseUsageNumber(value){
  if(value&&typeof value==='object'){
    // Usage stats store raw/real/weighted fractions; weighted is the best single number.
...
function extractUsageTable(raw){
  // Current stats files use a top-level `pokemon` object; older ones used `data`.
  const table=raw?.pokemon && typeof raw.pokemon==='object' ? raw.pokemon
    : raw?.data && typeof raw.data==='object' ? raw.data
    : {};
  const out: Record<string, any>={};
  for(const [name,entry] of Object.entries<any>(table)){
...
async function loadUsage(format){
  ...
  // cachedFetch avoids a network round trip for a multi-megabyte file that changes monthly at most.
  usagePromise[format]=cachedFetch(`https://pkmn.github.io/smogon/data/stats/${format}.json`,{maxAgeMs:7*86400000})
    .then(async r=>{
...
async function loadOfficialTierData(cfg){
  // Parse each species entry independently so one malformed/nested entry can't
  // carry its tier over to the next Pokémon.
  ...
  tierDataPromise[key]=cachedFetch(url,{maxAgeMs:7*86400000}).then(r=>{
    if(!r.ok) throw new Error(`Tier data request failed (${r.status})`);
    return r.text();
  }).then(text=>{
    const out: Record<string, any>={};
    // Species entries can contain nested objects (e.g. viableMoves), so a naive
    // "find next `},`" regex would swallow multiple species. Parse line-by-line instead.
    ...
    for(const line of lines){
      // Accept tabs or spaces for robustness to source formatting changes.
...
// formats-data.ts only tiers the *base* forme for species whose formes are
// mechanically identical (Arceus/Silvally types) - map e.g. "arceusbug" back to "arceus".
...
function officialTierOf(name,tierData,cfg){
  ...
  for(const id of candidates){
    ...
    // Illegal/LC/NFE aren't standard competitive viability tiers here.
...
function summarySeedFromTarget(t){
  // Deterministic seed from the Fakemon's own data only (not analysis settings like gen/pool).
  const stable={
    name:t?.name||'',
...
function makeCasualSummary(t,tf,tier,matchup,selectedFormat,statCombination,roleScore,typePct,cap,summarySeed){
  ...
  const stats=t.stats||{},bestStat=Object.entries<any>(stats).sort((a,b)=>Number(b[1])-Number(a[1]))[0];
  ...
  const roleEntries=Object.entries<any>(tf.roleSignals||{}).sort((a,b)=>b[1]-a[1]);
  ...
  if(bestStat&&Number(bestStat[1])>=110){
    ...
    topicPool.push(pick(v[STAT_NAMES[bestStat[0]].toLowerCase()]||Object.values<any>(v).flat()));
  }
...
function detailedProfile(mon){
  ...
  const unique=[...new Map<any, any>(moves.map(m=>[normalizeName(m.name),m])).values()];
  ...
  const roleSignals={
    ...
    physicalSweeper: physicalReady ? (Number(physicalReady)*18 + Math.max(0,atk-100)*0.45 + physical.length*5 + (fastEnough?10:0) + (setupSweeperEligible&&atk>=spa?8:0)) : 0,
    specialSweeper: specialReady ? (Number(specialReady)*18 + Math.max(0,spa-100)*0.45 + special.length*5 + (fastEnough?10:0) + (setupSweeperEligible&&spa>atk?8:0)) : 0,
...
function choosePrimaryRole(tf){
  ...
  const ranked=Object.entries<any>(rs)
...
function battleStatsFromBase(stats: Record<string, any>={}){
...
function strategicallyRelevantMoves(candidates){
  ...
  const groups={se:[] as any[],neutral:[] as any[],resisted:[] as any[]};
  ...
  Object.values<any>(groups).forEach(g=>g.sort(cmp));
  const selected: any[]=[], seen=new Set(), seenTypes=new Set();
...
function bestDamageOutput(attacker, defender, generation=9){
  ...
  if(!candidates.length)return {best:null as any,all:[] as any[],topMoves:[] as any[],hasAttack:false,bestSuperEffective:null as any,bestNeutral:null as any,bestResisted:null as any};
...
function tierComparablePool(pool, tierUsage, assignedTier, officialTiers, cfg){
  ...
  if(!wanted) return {pool:[] as any[],usage:{} as Record<string, any>};
...
function estimateTier(base,closest,tierUsage,officialTiers,targetProfile,cfg,matchup,roleScore,tf,metagameFit,intrinsic,abilityInfo){
  ...
  return{
    tier,score,reliability,anchor:anchor?Math.round(anchor.midpoint):null,usageEvidence:[] as any[],weighted:{} as Record<string, any>,
...
// ---- the Analysis tab ----
// What to compare against, and the latest result; js/app/components/editor/AnalysisTab.tsx draws both.
const DEFAULT_CFG={pool:'generation',gen:9,natdex:false,folder:'',region:'',comparePokemon:''};
let cfgState={...DEFAULT_CFG};
/** status: { kind: 'busy'|'ok'|'error', text }; result: the numbers the tab shows, or null while it works */
let view={status:{kind:'busy',text:'Loading data…'},result:null as any,error:''};

function getCfg(){return cfgState;}
function analysisView(){return view;}
function setView(patch){view={...view,...patch};notify();}

/** Changing what it compares against re-runs the analysis. */
function setAnalysisCfg(patch){
  cfgState={...cfgState,...patch};
  // a newly picked pool starts on its first choice, as a <select> would
  if(cfgState.pool==='region'&&!cfgState.region)cfgState.region=(api.getRegions?.()||[])[0]?.id||'';
  if(cfgState.pool==='folder'&&!cfgState.folder)cfgState.folder=(state.folders||[]).find(f=>f.type==='fakemon')?.id||'';
  if(cfgState.pool==='pokemon'&&!cfgState.comparePokemon){const p=uniqueDex()[0];cfgState.comparePokemon=p?(p.id||p.name):'';}
  scheduleAnalysis();
  notify();
}
function analysisPoolChanged(pool){setAnalysisCfg({pool});}

/** What the comparison pickers offer (the whole Pokédex only when that picker shows). */
function analysisChoices(withPokemon=false){
  return {
    regions:(api.getRegions?.()||[]).map(r=>({id:r.id,name:r.name})),
    folders:(state.folders||[]).filter(f=>f.type==='fakemon').map(f=>({id:f.id,name:f.name})),
    pokemon:withPokemon?uniqueDex().map(p=>({id:p.id||p.name,name:p.name})):[]
  };
}
...
/** Opening the tab: whatever was shown before is re-checked from scratch. */
function ensurePanel(){
  ...
  setView({status:{kind:'busy',text:state.sdLoaded?'Preparing analysis…':'Loading data…'},result:null as any,error:''});
  scheduleAnalysis();
}
async function runFakemonAnalysis(){
  ...
  if(key===lastAnalysisKey && (view.result||view.error))return;
  lastAnalysisKey=key;analysisBusy=true;
  const busy=text=>setView({status:{kind:'busy',text}});
  setView({result:null as any,error:''});
  busy('Preparing comparison…');
  try{
    ...
    busy('Calculating stat percentiles…');
    ...
    const roleValues=Object.entries<any>(tf.roleSignals||{}).sort((a,b)=>b[1]-a[1]);
    ...
    busy('Testing matchups across the selected metagame…');
    ...
    const matchup: Record<string, any>=buildMatchupProfile(target,matchupPool,matchupUsage,tf,cfg.gen);
    ...
    window.__lastAnalysis={
      ...
      usageFormats:Object.fromEntries(Object.entries<any>(tierUsage).map(([k,v])=>[k,Object.keys(v||{}).length])),
    ...
    const statPctForBullets=irrelevantOffenseKey
      ? Object.fromEntries(Object.entries<any>(statPct).filter(([k])=>k!==irrelevantOffenseKey))
      : statPct;
    Object.entries<any>(statPctForBullets).sort((a,b)=>b[1]-a[1]).slice(0,2).filter(x=>x[1]>=70).forEach(([k,v])=>strengths.push(`${STAT_NAMES[k]} is ${ordinal(v)} percentile`));
    Object.entries<any>(statPctForBullets).sort((a,b)=>a[1]-b[1]).slice(0,2).filter(x=>x[1]<=35).forEach(([k,v])=>weaknesses.push(`${STAT_NAMES[k]} is ${ordinal(v)} percentile`));
    if(metagameStatCombination!=null && metagameStatCombination>=70)strengths.push(`Stat profile is ${ordinal(metagameStatCombination)}-percentile quality among usage-weighted ${selectedFormat} Pokémon`);
    ...
    const loadedUsageCount=Object.values<any>(tierUsage).reduce((n,t)=>n+Object.keys(t||{}).length,0);
    const matchupRow=x=>({name:x.p.name,sprite:matchupSprite(x.p),scenario:matchupScenario(x),score:Math.round(x.score)});
    setView({
      status:{kind:'ok',text:`Analysis updated · ${pool.length} comparison Pokémon · ${metagameEntries.length} usage-weighted in ${selectedFormat} · ${loadedUsageCount} usage records loaded`},
      result:{
        tier:tier.tier,score:Math.round(tier.score),reliability:Math.round(tier.reliability),
        summary:makeCasualSummary(t,tf,tier,matchup,selectedFormat,statCombination,roleScore,typePct,intrinsic.cap,summarySeedFromTarget(t)),
        strengths,weaknesses,types:(t.types||[]).filter(Boolean),
        typePct:ordinal(typePct),typing:{weak:tf.typing.weak,resist:tf.typing.resist,immune:tf.typing.immune},
        tools:[tf.recoveryMoves?'Recovery':'',tf.pivot?'Pivot':'',tf.hazards?'Hazards':'',tf.removal?'Removal':'',tf.setup?'Setup':'',tf.statusUtility?'Status utility':''].filter(Boolean),
        roleScore:Math.round(roleScore),
        statCombination:Math.round(statCombination),
        pcts:{bulk:ordinal(bulkPct),offense:ordinal(offPct),speed:ordinal(speedPct),bst:ordinal(bstPct)},
        selectedFormat,metagameCount:metagameEntries.length,
        metagame:{
          stat:metagameStatCombination==null?'-':`${Math.round(metagameStatCombination)}/100`,
          bulk:metagameBulkPct==null?'-':ordinal(metagameBulkPct),
          offense:metagameOffPct==null?'-':ordinal(metagameOffPct),
          speed:metagameSpeedPct==null?'-':ordinal(metagameSpeedPct),
          matchups:matchup.weightedScore==null?'-':`${Math.round(matchup.weightedScore)}/100`
        },
        cap:{BSR:Math.round(intrinsic.cap.BSR),category:intrinsic.cap.category,PT:Math.round(intrinsic.cap.PT),ST:Math.round(intrinsic.cap.ST),PS:Math.round(intrinsic.cap.PS),SS:Math.round(intrinsic.cap.SS),ODB:intrinsic.cap.ODB.toFixed(1),PSB:intrinsic.cap.PSB.toFixed(1)},
        kitScore:Math.round(intrinsic.score),matchupScore:Math.round(matchup.weightedScore||50),
        usingComparableTier:!!matchup.usingComparableTier,
        good:(matchup.good||[]).map(matchupRow),bad:(matchup.bad||[]).map(matchupRow)
      },
      error:''
    });
  }catch(e: any){
    log.error('ANALYSIS', 'Analysis failed', e);
    setView({status:{kind:'error',text:'Analysis failed'},result:null as any,error:e.message||'Analysis failed.'});
  }finally{analysisBusy=false;}
}
function openAnalysisTab(){api.switchTab(null,'analysis');}
export {ensurePanel as renderAnalysis,runFakemonAnalysis,analysisPoolChanged,openAnalysisTab,scheduleAnalysis,getCfg as getAnalysisCfg,setAnalysisCfg,analysisView,analysisChoices};