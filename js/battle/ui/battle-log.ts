...
//
// Pure and presentational: no DOM, no app.ts, no battle state. It is handed
// the entry plus two lookups and returns an HTML string.

import { esc, toId } from '../../core/html.ts';
import {
...
export function formatLogLine(entry, nameFor, isMine) {
    ...
    const sd = (effect, situation, extra: Record<string, any> = {}, r = ref) =>
        paint(situationText(effect, situation, vars(extra, r)), r);
    // Showdown's generic template, the tier it falls back to itself.
    const gen = (key, extra: Record<string, any> = {}, r = ref) => paint(genericText(key, vars(extra, r)), r);
    ...
    const bySource = (name, { item, ability, move, plain }: { item?: any; ability?: any; move?: any; plain?: any }) => {
