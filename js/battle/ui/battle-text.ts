...
import { SD_VERSION } from '../engine/sd-facade.ts';
...
export function loadBattleText() {
    ...
    textPromise ||= (async () => {
        const parts = await Promise.all(PARTS.map(async name => {
            ...
            const table = Object.values<any>(module.exports)[0];
...
export function fillText(template, vars: Record<string, any> = {}) {
