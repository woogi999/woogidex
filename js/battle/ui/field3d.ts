...
// WebGL) mountField3D returns null and scene.ts falls back to the CSS field and
...
// URLs and effect names by scene.ts and draws them.

import { TYPE_COLORS } from './move-anim.ts';
...
// so the rest of the file (and scene.ts) is unchanged.
...
export async function mountField3D(fieldEl, fieldId) {
    ...
    function makeMon(which) {
        ...
        scene.add(mesh);
        return {
            mesh, mat, aspect: 1, base: anchors[which].clone(), anim: null as any,
            hidden: true, url: '', status: null as any
        };
    }
    ...
    function placeMon(m, { scale = 1, opacity = 1, tilt = 0, dx = 0, dy = 0, dz = 0 }: { scale?: any; opacity?: any; tilt?: any; dx?: any; dy?: any; dz?: any } = {}) {
    ...
    const hazards = { near: { key: '', group: null as any }, far: { key: '', group: null as any } };
    ...
    function applyHazards(which, conds: Record<string, any> = {}) {
    ...
    const api = {
        ...
        // version in move-anim.ts so scene.ts chooses the effect once and either
        // renderer can draw it.
        playFX(shape, { type = 'Normal', count = 8, fromSide = 'near', toSide = 'far' }: { type?: any; count?: any; fromSide?: any; toSide?: any } = {}) {
        ...
        destroy() {
            ...
            for (const g of Object.values<any>(hazardGeo)) g.dispose();
            ballGeo.dispose();
            ballTex?.dispose();
            scene.traverse(o => {
                o.geometry?.dispose?.();
