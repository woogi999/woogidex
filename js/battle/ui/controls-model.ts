...
export type ControlsKind = 'none'|'lead'|'desync'|'ended'|'animating'|'forced-switch' |'waiting-opponent-switch'|'switch'|'moves';
 ...
 */
export function controlsView(ui: Record<string, any> = {}): {kind: ControlsKind} {
...
export function switchOptions(side) {
    ...
    return side.team
        .map((m, i) => ({ m, i }))
        .filter(({ m, i }: { m?: any; i?: any }) => !m.fainted && i !== side.activeIndex);
}
 ...
 *
 */
export function connectionBanner(meta: Record<string, any> = {}): {tone: 'bad'|'', icon: string, label: string, note: string, canRetry: boolean}|null {
