...
export function shapeFor(move, type) {
    ...
    const hit = byName || SHAPE_RULES.find(([test]: any[]) => { try { return test(move || {}); } catch { return false; } })?.[1];
...
export function resolveMoveInfo(battle, sideIndex, moveName) {
    ...
    return { name: moveName, type: 'Normal', category: 'Physical', move: null as any };
}
...
function spawn(field, cls, styles, life) {
    ...
    for (const [k, v] of Object.entries<any>(styles)) {
 ...
 * @param els  scene element refs
 * @param attackerSlot  attacker's .ps-slot
 * @param targetSlot    defender's .ps-slot
 * @param info
 */
export function playMoveFX(els: {field:Element}, attackerSlot: Element, targetSlot: Element, info: { type: string; category: string; move?: any }) {
