...
//
// This is the same trick sd-abilities.ts plays, for the same reason. The move
...
// battle.ts at the points in the move pipeline where they belong. ~950 moves,
...
// bridged at all (see battle.ts `_moveEffects`).
...
let TABLE: any = null;
let MOVE_LIST: any = null;
let FINGERPRINT: any = null;

// The table is fetched from a PINNED release (see sd-facade.ts SD_VERSION),
...
// The handshake in battle-ui.ts is the backstop for whatever pinning misses.
...
// ---------------------------------------------------------------- compile
// Turns one Showdown move into { data, hooks, condition } for battle.ts.
...
// battle.ts keeps its fast path for the many moves that are just damage.
...
function invoke(fn, fx, hook, { battle, user, target, move, value, type }: { battle?: any; user?: any; target?: any; move?: any; value?: any; type?: any }, fallback) {
...
function callHook(fn, scope, hook, { self, foe, move, value, type }: { self?: any; foe?: any; move?: any; value?: any; type?: any }) {
    switch (hook) {
