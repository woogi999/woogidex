...
//
//   sd-abilities.ts  the simulator's ability table
//   sd-items.ts      the simulator's item table
//   raw code         an ability/move/item whose author wrote Showdown hooks by
//                    hand (see compileRawHooks below)
//   battle.ts        via rawMoveEffects, for a hand-written custom MOVE, which
...
import { pokeFacade, battleFacade, withEffect, Chain } from './sd-facade.ts';
...
// ==================== the hook table ====================
// event : the engine event this hook binds to (see battle.ts runEvent).
...
// the move executor in battle.ts, which calls them at named points in the
// pipeline (see sd-moves.ts runMoveHook). Raw code on a CUSTOM MOVE is routed
...
export const HOOK_EVENTS = Object.fromEntries(
    Object.entries<any>(HOOK_SPECS).map(([hook, spec]) => [hook, spec.event])
);
...
// fire it twice.
export function compileHookTable(entry, label = '', { isItem = false, skip = null }: { isItem?: any; skip?: any } = {}) {
    if (!entry || typeof entry !== 'object') return null;
    const out: Record<string, any> = {};

    for (const [hook, spec] of Object.entries<any>(HOOK_SPECS)) {
...
export function compileRawHooks(source, label = '', { isItem = false, kind = 'ability' }: { isItem?: any; kind?: any } = {}) {
...
// showdownMoveEffects() returns, so battle.ts can run a hand-written move down
...
export function rawMoveEffects(source, name = '', dataFields: any[] = []) {
    const parsed = parseRawEntry(source);
...
export function parseRawEntry(source) {
    ...
    if (!text) return { ok: true, entry: null as any };
...
export function checkRawHooks(source, kind = 'ability') {
    const text = String(source || '').trim();
    if (!text) return { ok: true, hooks: [] as any[], unknown: [] as any[], data: [] as any[] };

    const parsed = parseRawEntry(text);
    if (!parsed.ok) return { ok: false, error: parsed.error, hooks: [] as any[], unknown: [] as any[], data: [] as any[] };
