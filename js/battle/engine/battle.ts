...
// divergence is a desync (see net/protocol.ts, which hashes state each turn).
//
// Pure module: no DOM, no app.ts. Node can run it directly for tests.

import { BattleRNG } from './rng.ts';
import { calcStat, typeEffectiveness, toId, BOOST_IDS } from './dex.ts';
import { compileProgram } from './ability-runtime.ts';
import { builtinHandlers } from './builtin-abilities.ts';
import { showdownAbilityHandlers } from './sd-abilities.ts';
import { showdownItemHandlers } from './sd-items.ts';
import { compileRawHooks, rawMoveEffects } from './sd-hooks.ts';
import { showdownMoveEffects, runMoveHook, runConditionHook, DATA_FIELDS } from './sd-moves.ts';
import { builtinItemHandlers } from './builtin-items.ts';
import {
...
// differently. The handshake (net/protocol.ts) refuses a battle across a
...
export const STRUGGLE = Object.freeze({
    ...
    drain: null as any, recoil: null as any, heal: null as any, status: null as any, volatileStatus: null as any,
    boosts: null as any, secondaries: Object.freeze([]), multihit: null as any,
    weather: null as any, terrain: null as any, sideCondition: null as any, selfSwitch: null as any,
    self: null as any, willCrit: false, damage: null as any, forceSwitch: false,
    thawsTarget: false, sleepUsable: false, breaksProtect: false, ohko: false,
    ignoreImmunity: true, overrideOffensiveStat: null as any, overrideDefensiveStat: null as any,
    program: null as any, struggle: true
});
...
// move bridge (sd-moves.ts DATA_FIELDS); this list is the fallback for when
...
export class Battle {
    ...
    // turn (see net/protocol.ts). Lockstep is only meaningful when both ends
    ...
    constructor({ seed, dex, sides, format = 'singles', timerMs = 150000,
                  ...
                  caps = { sdMoves: true, sdAbilities: true } }: { seed?: any; dex?: any; sides?: any; format?: any; timerMs?: any; trustLocalCode?: any; caps?: any }) {
        ...
        this._callDepth = 0;
        // Showdown's move handlers, compiled once per move id (see sd-moves.ts).
        ...
        this.field = { weather: null as any, weatherTurns: 0, weatherSource: null as any, terrain: null as any, terrainTurns: 0 };
    ...
    _buildSide(raw, index) {
        const side = {
            ...
            team: [] as any[], activeIndex: 0, conditions: {} as Record<string, any>
        };
    ...
    _buildPokemon(entry, side, slot) {
        ...
        const pokemon = {
            ...
            volatiles: {} as Record<string, any>,
            moves,
            ability,
    ...
    // over the wire (see the ruleset handshake in battle-ui.ts) and lands here.
    ...
    // ==================== state mutation API ====================
    // Everything below is what ability-runtime.ts (and the move executor)
    ...
    damage(amount, target, source: any = null, why = '') {
    ...
    boost(target, boosts, source: any = null, why = '') {
        ...
        const vetted = this.runEvent('tryBoost', target, { value: requested, move: null as any });
        if (vetted === false) return;
        const applied = (vetted && typeof vetted === 'object') ? vetted : requested;
        const landed: Record<string, any> = {};
        for (const [stat, delta] of Object.entries<any>(applied)) {
        ...
        if (Object.keys(landed).length) this.runEvent('afterBoost', target, { value: landed, move: null as any });
    }
    ...
    trySetStatus(target, status, source: any = null, why = '') {
        ...
        if (this.runEvent('trySetStatus', target, { status, move: null as any }) === false) {
    ...
    setTypes(target, types, why = '', { quiet = false }: { quiet?: any } = {}) {
        if (!target) return;
    ...
    addVolatile(target, id, data: Record<string, any> = {}) {
    ...
    setWeather(weather, source: any = null, why = '') {
        if (!weather) {
    ...
    setTerrain(terrain, source: any = null, why = '') {
        if (!terrain) {
    ...
    // trust the code it is holding. See sd-hooks.ts compileRawHooks for why
    // that gate exists and why both clients of a PvP battle skip it together.
    _rawHandlers(entity, opts: Record<string, any> = {}) {
    ...
    _mergeHandlers(a, b) {
        if (!b) return a || {};
        const merged = { ...(a || {}) };
        for (const [event, handlers] of Object.entries<any>(b)) {
            merged[event] = [...(merged[event] || []), ...handlers];
        }
        return merged;
    }

    runEvent(eventName, pokemon, extra: Record<string, any> = {}) {
    ...
    // ==================== stats ====================
    getStat(pokemon, stat, { ignoreBoosts = false, crit = false }: { ignoreBoosts?: any; crit?: any } = {}) {
    ...
    // hand-written hooks. Moves could carry a program (dex.ts has read one off
    ...
    _runMoveEvent(event, user, target, extra: Record<string, any> = {}) {
        ...
        const ctx = {
            ...
            stat: null as any, crit: !!extra.crit, eff: extra.eff ?? null,
            status: null as any, role: null as any, whose: null as any,
            value: extra.value, vars: {} as Record<string, any>, immune: false
        };
        let result = extra.value;
        for (const h of handlers) {
            const out = h(ctx);
    ...
    useMove(moveOrId, user, target: any = null) {
    ...
    _executeMove(user, slot, forcedTarget: any = null) {
        try {
    ...
    _runMove(user, slot, forcedTarget: any = null) {
        ...
        let acting = target;
        const hook = (name, extra: Record<string, any> = {}, fallback: any = undefined) =>
    ...
    _isSemiInvulnerable(target, move) {
        ...
        return runConditionHook(fx, 'onInvulnerability',
            { battle: this, user: target, target: null as any, move }, undefined) === false;
    }
    ...
    _effectiveness(move, target, fx) {
        ...
        for (const t of target.types) {
            ...
            let out = runMoveHook(fx, 'onEffectiveness',
                { battle: this, user: null as any, target, move, value: exponent, type: t }, exponent);
    ...
    // Damage runs through mechanics.ts, which follows mainline's order and its
    // fixed-point rounding rather than multiplying floats and flooring.
    _calcDamage(user, target, move, eff, fx: any = null) {
        ...
        let damage = calcDamage({
            ...
            // note in mechanics.ts calcDamage.
    ...
    _applyStatusMove(user, target, move, fx: any = null) {
        let did = false;
    ...
    // player which of the four went wrong. See net/protocol.ts setupMismatch.
    //
...
function markAsItem(handlers) {
    for (const list of Object.values<any>(handlers || {})) for (const h of list) h.isItem = true;
    return handlers;
}
