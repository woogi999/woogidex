...
// ability-blocks.ts already stores every custom ability as a plain-object AST:
//
...
// the two export paths changes. Adding a new block to ability-blocks.ts means
...
//
// Deliberately dependency-free (no app.ts / no DOM) so it runs in Node tests.

import { toId } from './dex.ts';
...
const TRIGGER_MAP = {
    ...
    // `self` (see battle.ts _runMoveEvent), so these two bind to the same event
    ...
    // (see battle.ts getStat) -- and a block program means the first of those.
    ...
    // the engine is holding, which is the per-use copy (battle.ts _activeMove),
...
// Mirrors amountExpression() in ability-blocks.ts so the interpreted result
...
// ---------------------------------------------------------------------------
// condition semantics - keyed by the same ids as CONDITIONS in ability-blocks.ts
// ---------------------------------------------------------------------------
...
// ability-blocks.ts, which is the list the editor offers.
...
// Mirrors pokemonPropertyExpression() in ability-blocks.ts, so the interpreted
...
function pokemonProperty(mon, property) {
    ...
    const boosts = Object.values<any>(mon.boosts || {});
    switch (prop) {
...
// ---------------------------------------------------------------------------
// action semantics - keyed by the same ids as ACTIONS in ability-blocks.ts
// ---------------------------------------------------------------------------
...
const ACTION_RUNTIME = {
    ...
    // These three write to the active move copy (battle.ts _activeMove), so a
...
export function compileProgram(program, abilityName = '') {
    ...
    // exactly like normalizeAbilityBlocks() in ability-blocks.ts does.
...
// Call with the ACTIONS/CONDITIONS objects from ability-blocks.ts.
export function coverageReport(ACTIONS: Record<string, any> = {}, CONDITIONS: Record<string, any> = {}, TRIGGERS: Record<string, any> = {}) {
    return {
