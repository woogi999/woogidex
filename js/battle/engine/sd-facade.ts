...
// bridge (sd-abilities.ts) and the move bridge (sd-moves.ts) speak to one
...
//
// Pure module: no DOM, no app.ts, no network beyond loadShowdownTable's fetch.

import { cachedFetch } from '../../core/net-cache.ts';
import { toId } from '../../core/html.ts';
import { modify, Chain } from './mechanics.ts';
import { typeEffectiveness } from './dex.ts';
import { showdownMoveEntry, showdownMoveList } from './sd-moves.ts';
...
// Built on demand rather than as a module-level constant: sd-moves.ts imports
...
// FNV-1a, the same hash battle.ts uses for its per-turn state. Cheap, stable
...
function fieldFacade(battle) {
    return {
        ...
        pseudoWeather: {} as Record<string, any>,
...
export function pokeFacade(mon, battle) {
    ...
    const f = {
        ...
        getAbility: () => ({ id: toId(mon.ability?.name || ''), name: mon.ability?.name || '', flags: {} as Record<string, any> }),
        ...
        positiveBoosts: () => Object.values<any>(mon.boosts).reduce((n, v) => n + (v > 0 ? v : 0), 0),
        ...
        setBoost: (boosts) => { for (const [k, v] of Object.entries<any>(boosts || {})) if (k in mon.boosts) mon.boosts[k] = v; },
        ...
        disableMove: (id) => {
            ...
            // again next turn -- see battle.ts runTurn.
...
export function battleFacade(battle, effectState, chain: any = null) {
    return {
        ...
        // turn. battle.ts keeps the remaining ordered actions in _queue.
        queue: {
        ...
        dex: {
            moves: {
                get: (m) => showdownMoveEntry(m) || { id: toId(m), flags: {} as Record<string, any> },
                all: () => showdownMoveList()
            },
            abilities: { get: (a) => ({ id: toId(a), name: String(a || ''), flags: {} as Record<string, any> }) },
            items: { get: (i) => ({ id: toId(i), name: String(i || ''), isBerry: /berry$/i.test(String(i || '')), flags: {} as Record<string, any> }) },
            species: { get: (sp) => (typeof sp === 'object' ? sp : { id: toId(sp), name: String(sp || ''), types: [] as any[] }) },
            types: {
                get: (t) => ({
                    id: toId(t), name: String(t || ''),
                    damageTaken: {} as Record<string, any>,
