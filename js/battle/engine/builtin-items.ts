...
// seam abilities use (see battle.ts runEvent / builtin-abilities.ts). The
...
import { modify } from './mechanics.ts';
...
const STATUS_BERRY = {
    ...
    rawstberry: 'brn', aspearberry: 'frz', persimberry: null as any   // confusion not modeled
};
const ITEM_LABEL = {
...
const ITEM_IMPLS: Record<string, any> = {
    // ---------- passive residual ----------
    leftovers: {
...
for (const [id, status] of Object.entries<any>(STATUS_BERRY)) {
    if (!status) continue;
    ITEM_IMPLS[id] = {
        setStatus: (_p, ctx) => {
...
export function builtinItemHandlers(itemName) {
    ...
    for (const [event, fn] of Object.entries<any>(impl)) {
