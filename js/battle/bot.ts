...
import { typeEffectiveness } from './engine/dex.ts';
import { canSelectMove, STRUGGLE_INDEX } from './engine/battle.ts';
...
export function chooseBotAction(battle, sideIndex) {
    ...
    const options = usable.map(({ slot, index }: { slot?: any; index?: any }) => ({
...
function predictThreat(battle, foe, target) {
    if (!foe || !target) return { damage: 0, kills: false, type: null as any };
    let worst = { damage: 0, kills: false, type: null as any };
...
function pickSwitchTarget(battle, sideIndex, foe, exclude: any = null) {
    const side = battle.sides[sideIndex];
