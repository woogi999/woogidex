...
import { toId } from './engine/dex.ts';
...
export function monForMember(member, fakemonDB: any[] = []) {
...
export function checkTeamReady(team, fakemonDB: any[] = []) {
    const errors: any[] = [];
...
export function toPeerPackage(team, fakemonDB: any[] = [], libs: Record<string, any> = {}) {
...
export function toServerSnapshot(pkg) {
    return {
        ...pkg,
        members: (pkg.members || []).map(({ artwork, shinyArtwork, ...rest }: any) => rest)
    };
}
...
export function mergeDexPayload(pkgA, pkgB, local: Record<string, any> = {}) {
    const dedupe = (...lists) => {
        const out = new Map();
