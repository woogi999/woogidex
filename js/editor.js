        ...
        async function fetchShowdownData() {
            ...
            try {
                ...
                for (const [key, m] of Object.entries(movesRaw)) {
                    if (m.isNonstandard === 'Past' || m.isNonstandard === 'Future') continue;
                    state.sdMoves[key] = {
                        name: m.name || key,
...
// ==================== CUSTOM MOVES ====================
        ...
        function saveCustomMove() {
            ...
            const editIndex = document.getElementById('custom-move-edit-index').value;
            ...
            closeModal('custom-move-modal'); sortLearnset(); renderLearnset(); updatePreview(); api.autoSave(); api.saveToStorage(); api.renderCollection();
        ...
        function showDetailPopup(title, content) {
            ...
            popup.innerHTML = `
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
                    <h3>${title}</h3>
                    <button class="modal-close" onclick="this.closest('.move-detail-popup').remove(); document.querySelector('.overlay-dark').remove();">&times;</button>
                </div>
                ${content}
            `;
