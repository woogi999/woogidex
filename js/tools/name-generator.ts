// Pokémon/Fakemon-style name generator: keeps semantic roots from the user's
// traits, turns them into short stems, blends/mutates them into candidates,
// then scores for memorability, pronunciation, and creature-name feel.
...
// onomatopoeic roots: a second naming style based on sound/movement/cry
const SOUND_BY_TYPE = {
...
// extra creature/theme vocabulary; roots kept short and name-friendly rather
// than full dictionary words
...
// conservative transformations: preserve recognizable roots, not random noise
const SOUND_SWAPS = [
...
function stem(root, rng) {
    ...
    // strip dictionary-like tails while retaining the recognizable core
    s = s.replace(/(ing|ed|ly|ness|ous|ful|tion|ment)$/i, '');

    // prefer a compact 2-6 letter stem, cutting after a vowel when possible
    if (s.length > 6) {
...
function flattenWeightedConcepts(pools, rng) {
    // one or two roots are enough; three tends to feel stuffed
    const unique: any[] = [];
    const seen = new Set();
...
function blendRoots(a, b, rng) {
    ...
    // longest shared suffix/prefix, only accepting meaningful overlap
    let best = 0;
    ...
    if (av && bv) {
        // usually retain the first vowel, start the second root at its next consonant
        let j = 0;
    ...
    if (!av && !bv) {
        // both seams hard: keep only one consonant from the second root
...
function mutateName(name, rng) {
    ...
    // gentle vowel variation, never wholesale syllable replacement
    if (rng() < 0.22) {
...
function scoreName(name, roots, traits) {
    ...
    // strong preference for the compact 2-3 syllable zone
    ...
    // reward pieces of the actual concepts, keeping the name anchored
    ...
    // avoid names too literal/dictionary-like
...
function buildSoundCandidate(soundPools, conceptPools, rng) {
    ...
    if (concepts.length && rng() < 0.82) {
        ...
        if (concept) {
            ...
            // prefer sound + concept since it preserves the audible hook
            result = rng() < 0.68
    ...
    // give a sound-root a small species-name ending (e.g. Buzz -> Buzza)
...
function buildCandidate(pools, rng) {
    ...
    // occasionally add a tiny vowel onset to avoid a harsh start
...
function generateFakemonNames(traits: Record<string, any> = {}, count = 5, seed: any = null) {
    ...
    for (let i = 0; i < attempts; i++) {
        // onomatopoeia is a subtle accent, not the primary naming style
        ...
        if (candidate.sound) {
            score += 2;
            // reward sound-like spellings without letting them dominate
    ...
    // allow at most one subtle sound-inspired name into a larger suggestion set
...
function generateFakemonName(traits: Record<string, any> = {}, seed: any = null) {
