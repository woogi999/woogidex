...
import { sortLearnsetEntries, normalizeMoveCategoryInput, normalizeMoveTypeInput } from '../editor/editor.js';
...
let pendingCollectionImportFile = null;
        ...
        // compatibility alias for older inline handlers
        ...
        function collectFamilyFakemon(root) {
            const byId = new Map((state.fakemonDB || []).map(f => [String(f.id), f]));
            const out = new Map();
            const queue = [root];
            while (queue.length) {
        ...
        function collectReferencedCustomLibraries(list) {
            ...
            const pick = (bucket, source, { id, name }) => {
