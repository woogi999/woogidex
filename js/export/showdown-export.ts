...
import { toId as toShowdownId } from '../core/html.ts';
        ...
        function buildShowdownEvolutionContext(fakemonList, graphOverride: any = null) {
            const fakemonIds = new Map<any, any>((fakemonList || []).map((f, i) => [String(f.id), toShowdownId(f.name) || `fakemon${i + 1}`]));
            const graph = graphOverride || fakemonList?.find(f => f?.evolutionGraph)?.evolutionGraph || state.evolutionGraph || null;
            if (!graph?.nodes) return { graph: null as any, speciesByNode: new Map(), relations: [] as any[] };
        ...
        function applyEvolutionToSpeciesEntries(entries, fakemonList, speciesIds, graphOverride: any = null) {
        ...
        function buildPokedexTs(fakemon, speciesId) {
            ...
            const entries = new Map<any, any>([[speciesId, entry]]);
