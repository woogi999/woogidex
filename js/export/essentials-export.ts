...
// generates PBS text files for Pokemon Essentials (v19+ split-file format:
// pokemon.txt, pokemon_metrics.txt, pokemon_dexentries.txt), same pattern as
// showdown-export.ts. Custom moves/abilities are skipped from the mechanical
// data since Essentials needs actual script-level effect code, not just a name.
        ...
        function buildEssentialsGenderRatio(genderRatio) {
            ...
            // snap to the nearest standard Essentials gender-ratio bucket
            const buckets = [
        ...
        function essentialsMoveToken(entry) {
            // no explicit method is treated as a tutor move so it's still teachable
        ...
        function getEvolutionPathsForFakemon(fakemon, fakemonList, internalNames) {
            ...
            return owner ? getEvolutionPathsForNode(graph, owner, fakemonList, internalNames) : {paths:[] as any[], notes:[] as any[]};
        }
        ...
        function buildSpeciesBlock(fakemon, internalName, evolutionContext: any = null) {
            ...
            const levelMoves: any[] = [];
            const eggMoves: any[] = [];
            const tutorMoves: any[] = [];
            learnset.forEach(m => {
            ...
            const evo = evolutionContext ? getEvolutionPathsForFakemon(fakemon, evolutionContext.fakemonList, evolutionContext.internalNames) : {paths:[] as any[],notes:[] as any[]};
...
`;
        ...
        async function exportEssentialsMod() {
            try {
                ...
                // prevos/evolutions/megas/formes are separate Fakemon; PBS needs a
                // real entry for each or the evolution lines point at nothing
                const family = api.buildFakemonExportBundle?.(fakemon)?.fakemonDB || [fakemon];
                const internalNames = makeUniqueInternalNames(family);
                const internalName = internalNames[0];
                const pokemonBlocks: any[] = [], metricsBlocks: any[] = [], dexBlocks: any[] = [];
                family.forEach((f, i) => {
                    const { block } = buildSpeciesBlock(f, internalNames[i], { fakemonList: family, internalNames });
                ...
                const abilitiesScript = api.buildEssentialsAbilitiesFile ? api.buildEssentialsAbilitiesFile(family) : null;
                if (abilitiesScript) zip.file('Woogidex_Abilities.txt', abilitiesScript);
                zip.file('README.txt', buildReadmeTxt(fakemon));
                const evolutionText = buildEvolutionNotesText(family);
                if (evolutionText) zip.file('evolution_notes.txt', evolutionText);
                const graph = getEssentialsEvolutionGraphForFakemon(fakemon);
                const vanillaOverrides = buildVanillaEvolutionOverrides(graph, family, internalNames);
        ...
        async function exportCollectionAsEssentialsMod() {
            try {
                ...
                let totalSkipped = 0;
                const pokemonBlocks: any[] = [];
                const metricsBlocks: any[] = [];
                const dexBlocks: any[] = [];
                const evolutionNotes: any[] = [];
