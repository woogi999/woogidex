...
// since it evicts gracefully under storage pressure and net-cache.ts already
...
/**
 * @param id published_mons row id
 * @returns the stored data URI, or null on a miss
 */
export async function getCachedArt(id: string): Promise<string|null> {
    if (!id) return null;
 ...
 * @param dataUri
 */
export async function putCachedArt(id: string, dataUri: string) {
    if (!id) return;
    const cache = await openCache();
    if (!cache) return;
    try {
