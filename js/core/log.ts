// central diagnostics log; other modules should use log.* instead of console.* directly
...
function readBool(key, fallback = true) {
    ...
    } catch (_: any) { return fallback; }
}

const bootTime = typeof performance !== 'undefined' ? performance.now() : Date.now();
const config: { enabled: boolean; verbose: boolean; minLevel: string; traceScope?: string | null } = { enabled: readBool(STORAGE_KEY, true), verbose: readBool(VERBOSE_KEY, true), minLevel: 'debug' };
const history: any[] = [];
let context = { state: null as any, api: null as any };
...
function safeClone(value) {
    ...
    try { return JSON.parse(JSON.stringify(value)); } catch (_: any) { return value; }
}
...
function findMove(name) {
    ...
    const exact = Object.values<any>(moves).find(m => String(m.name || '').toLowerCase() === needle);
    if (exact) return exact;
    const custom = (state.learnset || []).find(m => String(m.name || '').toLowerCase() === needle);
    if (custom) return custom;
    return Object.values<any>(moves).find(m => String(m.name || '').toLowerCase().includes(needle)) || null;
}
...
function run(command, ...rest) {
    ...
    try {
        switch (cmd) {
            ...
            case 'flags': { const m = findMove(arg); if (!m) return out('Move not found'); const sd = Object.values<any>(state?.sdMoves || {}).find(x => x.name === m.name); return out(`Flags: ${m.name}`, { stored: m.flags, showdownFlags: sd?.flags, multihit: sd?.multihit ?? m.multihit, pivot: m.pivot }); }
            ...
            case 'eval': {
                ...
                try {
                    ...
                    catch (_: any) { value = Function('window','state','api','log', `"use strict"; ${expr}`)(window, state, api, log); }
            ...
            case 'modules': return out('Loaded modules', [...document.querySelectorAll<HTMLScriptElement>('script[type=module][src]')].map(s => s.src));
            case 'network': return out('Recent network resources', performance?.getEntriesByType?.('resource')?.slice(-50).map(r => ({name:r.name,duration:r.duration,size:(r as PerformanceResourceTiming).transferSize||0}))); 
            case 'memory': { const mem = (performance as any)?.memory; return out('Memory usage', mem ? {usedJSHeapSize: mem.usedJSHeapSize, totalJSHeapSize: mem.totalJSHeapSize, jsHeapSizeLimit: mem.jsHeapSizeLimit} : 'performance.memory unavailable'); }
            ...
            case 'clear': try { console.clear(); } catch (_: any) {} return out('Console clear requested');
...
export const log = {
    ...
    setEnabled(value) { config.enabled = !!value; try { localStorage.setItem(STORAGE_KEY, String(config.enabled)); } catch (_: any) {} emit('info','LOGGER',`logging ${config.enabled ? 'enabled' : 'disabled'}`); },
    setVerbose(value) { config.verbose = !!value; try { localStorage.setItem(VERBOSE_KEY, String(config.verbose)); } catch (_: any) {} emit('info','LOGGER',`verbose logging ${config.verbose ? 'enabled' : 'disabled'}`); },
