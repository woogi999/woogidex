...
// js/core/protect.ts for right-click/drag/copy.
//
 ...
 *
 * @param dataUri the artwork: a data: URI, or masked (see maskedArtwork)
 * @param {{alt?: string, className?: string}} options
 * @returns canvas markup, or '' when there is no artwork to shield
 */
export function shieldedArtHtml(dataUri: string, { alt = '', className = '' }: { alt?: any; className?: any } = {}): string {
 ...
 * @param envelope
 * @returns '' when the envelope is empty or malformed
 */
export function maskedArtwork(envelope: {mime?: string, key?: string, iv?: string, body?: string}|null): string {
    if (!envelope) return '';
 ...
 * canvas painter here, the thumbnail maker in community.ts) do not each have to
 * know whether they were handed a data: URI or a masked envelope.
 *
 * @param source a data: URI, or masked artwork
 * @returns null when there is nothing to decode
 */
export async function artworkBlob(source: string): Promise<Blob|null> {
 ...
 *
 * @param source
 * @returns the original string if it was not masked, '' on failure
 */
export async function artworkDataUri(source: string): Promise<string> {
    const value = String(source || '');
 ...
 * @param source a data: URI, or masked artwork
 */
export async function frameCount(source: string): Promise<number> {
 ...
 */
export async function paintShieldedCanvas(canvas: HTMLCanvasElement, source: string): Promise<boolean> {
