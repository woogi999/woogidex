...
import { form, heightDisplay, weightDisplay } from '../editor/draft.ts';
import type { Fakemon } from '../app/types.ts';

// ==================== IndexedDB ====================
        const IDB_NAME = 'woogidex-db';
        const IDB_VERSION = 1;
        const IDB_STORE = 'kv';
        let idbPromise: any = null;
...
// differ from what the template filled in (js/features/regions.ts). This is
...
let lastSeenStamp: any = null;       // the stamp this session last read or wrote
 ...
 * holding something. Drives the warning modal in js/features/recovery.ts.
 */
...
/** The last known-good collection snapshot, or null. Read by js/features/recovery.ts. */
...
function normalizeCollectionArray(arr, options: Record<string, any> = {}) {
    ...
    reattachOrphanedFolderItems();
        ...
        function autoSave(immediate = false) {
            ...
            const doSave = async () => {
                ...
                const fakemon: Fakemon | null = buildFakemonObject();
        ...
        function buildFakemonObject() {
            ...
            return {
                ...
                // the region, from the editor's Add to region button (js/features/regions.ts)
                ...(() => { const ids = (form.regionIds || []).filter(id => api.getRegions?.().some(r => String(r.id) === String(id))); return { regionId: ids[0] || null, regionIds: ids }; })(),
                ...
                isMega: !!form.isMega,
                isFormeChange: !!form.isForme,
                type1: form.type1,
                type2: form.type2,
                number: form.number.trim(),
                level: parseInt(String(form.level)) || 100,
                stats: {
                ...
                abilities: state.abilities.filter(a => a.name && a.name.trim()).map(a => {
                    const entry: { name: string; source: string; desc: string; customId?: string } = { name: a.name.trim(), source: a.source || 'sd', desc: a.desc || a.description || '' };
                    if (a.customId) entry.customId = a.customId;
                    return entry;
                }),
                dexEntry1: form.dexEntry1.trim(),
                dexEntry2: form.dexEntry2.trim(),
...
// ==================== save / load ====================
        ...
        // (js/features/cloud-save.ts), which is manual, never automatic.
        async function saveToStorage(options: Record<string, any> = {}) {
            ...
            // changed, stay out of storage (see js/features/regions.ts)
        ...
        async function loadFromStorage() {
            ...
            for (;;) {
                try {
                    ...
                    if (Array.isArray(data)) {
                        ...
                        // recovery.ts, which treats it as an independent backup copy
            ...
            wipeSuspected = state.fakemonDB.length === 0 && (marker ?? 0) > 0;
            if (wipeSuspected) {
        ...
        function migrateCustomLibrariesFromCollection() {
            const moveMap = new Map<any, any>((state.customMoves || []).filter(m => m && m.id).map(m => [m.id, m]));
            const abilityMap = new Map<any, any>((state.customAbilities || []).filter(a => a && a.id).map(a => [a.id, a]));
            const itemMap = new Map<any, any>((state.customItems || []).filter(i => i && i.id).map(i => [i.id, i]));
