...
// painted before the contests table is queried, so a deep link shows skeleton
// cards instead of a blank pane until loadEventsView() replaces it
function renderEventsSkeleton() {
    const list = document.getElementById('events-list');
    if (!list) return;
    const status = document.getElementById('events-status');
    if (status) status.textContent = '';
    const card = () => `<div class="contest-card skel-card">
        <span class="skel skel-text" style="width:55%;height:15px;margin-bottom:10px;"></span>
        <span class="skel skel-text" style="width:85%;"></span>
        <span class="skel skel-text" style="width:40%;"></span>
      </div>`;
    list.innerHTML = Array.from({ length: 2 }, () => `
      <section class="event-group">
        <div class="event-card-head">
          <div>
            <span class="skel skel-text" style="width:180px;height:18px;margin-bottom:6px;"></span>
            <span class="skel skel-text" style="width:140px;height:11px;"></span>
          </div>
        </div>
        <div class="contest-list">${card()}${card()}</div>
      </section>`).join('');
}

// events is a panel of the community hub, not its own tab; /events still
// resolves as a route but opens the hub on this panel
async function openEvents() {
    await api.openCommunityHub?.({ panel: 'events' });
    if (typeof lucide !== 'undefined') lucide.createIcons();
}

// kept apart from openEvents() so switching panels doesn't re-run hub navigation
...
// separate from rendering since the hub landing page also wants this data,
// and it has no #events-list to render into
async function fetchEvents() {
    const client = await api.getClient();
    const [{ data: events, error }, { data: contests, error: ce }, { data: subs, error: se }] = await Promise.all([
    ...
    EVENTS_STATE.loaded = true;
    return EVENTS_STATE.events;
}

/** Contests open for entries or votes right now, newest first. */
...
/** Loaded contests, for a caller that wants them without triggering a fetch. */
function getLoadedEvents() { return EVENTS_STATE.events; }
...
function renderEventGroup(e) {
    ...
    return `
      <section class="event-group${live ? ' is-live' : ''}">
        <div class="event-card-head">
          <div>
            <div class="event-group-title-row">
              <h3>${esc(e.title)}</h3>
              ${live ? '<span class="event-live-pill"><span class="event-live-dot"></span> Live now</span>' : ''}
            </div>
            ${e.description ? `<div class="event-card-description">${esc(e.description)}</div>` : ''}
            <div class="event-date"><i data-lucide="calendar"></i> ${fmt(e.starts_at)} to ${fmt(e.ends_at)}</div>
          </div>
          <div class="event-group-count">${contests.length} contest${contests.length === 1 ? '' : 's'}</div>
        </div>
        <div class="contest-list">${contests.length
            ? contests.map(renderContest).join('')
            : '<div class="community-empty">No contests in this event yet.</div>'}</div>
      </section>`;
}
...
function renderEvents() {
    ...
    const live = EVENTS_STATE.events.filter(eventIsLive);
    const past = EVENTS_STATE.events.filter(e => !eventIsLive(e));
    list.innerHTML = [...live, ...past].map(renderEventGroup).join('');
    if (typeof lucide !== 'undefined') lucide.createIcons();
}
...
// `artwork` is a plain base64 data-URI string, not an object with .normal/.url
...
window.openEvents=openEvents;window.getLiveContests=getLiveContests;window.showEventsPanel=showEventsPanel;window.loadEventsView=loadEventsView;window.toggleContestDetails=toggleContestDetails;window.toggleContestResults=toggleContestResults;window.retryContestResults=retryContestResults;window.submitContestEntry=submitContestEntry;window.withdrawContestEntry=withdrawContestEntry;window.openContestMonPicker=openContestMonPicker;window.filterContestMonPicker=filterContestMonPicker;window.chooseContestMon=chooseContestMon;window.openContestSignIn=openContestSignIn;window.startContestVoting=startContestVoting;window.jumpContestVote=jumpContestVote;window.setContestVoteScore=setContestVoteScore;window.setContestVoteRemarks=setContestVoteRemarks;window.saveAndNextContestVote=saveAndNextContestVote;window.submitContestBallot=submitContestBallot;window.closeContestVoting=closeContestVoting;

// also export through the module system, not just window.* above - app.js
// merges these into the shared `api` object for its router to call
export {
    openEvents, showEventsPanel, fetchEvents, getLiveContests, getLoadedEvents, renderEventsSkeleton, loadEventsView, toggleContestDetails, toggleContestResults, retryContestResults,
