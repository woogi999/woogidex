 ...
 * @param openFn decides for itself whether to open a modal
 */
export function queueAutoModal(openFn: () => void) {
    queue.push(openFn);
    pump();
}
...
function pump() {
    ...
    while (queue.length && !anyModalOpen()) {
        ...
        try { openFn(); } catch (e: any) { console.error('[MODAL-QUEUE] auto-open failed', e); }
    }
