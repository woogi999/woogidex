...
let standing: any = null;          // last my_standing() result
 ...
 */
async function refreshStanding(force = false): Promise<{ banned: boolean; muted: boolean; banned_until: any; muted_until: any; ban_reason: any; mute_reason: any; warnings: number } | null> {
 ...
 * @param context  where it came from, e.g. 'comment' - shown to staff
 */
async function screenContent(text: string, context: string = 'content') {
    const body = String(text || '').trim();
    if (!body) return { allowed: true, matches: [] as any[] };
    try {
        const client = await getClient();
        const { data, error } = await client.rpc('screen_content', { p_text: body, p_context: context });
        if (error) throw error;
        const verdict = data || { allowed: true, matches: [] as any[] };
        if (verdict.punishment) invalidateStanding();
        return verdict;
    } catch (e: any) {
        log.warn?.('MOD', 'Content screening unavailable, deferring to the server trigger', e);
        return { allowed: true, matches: [] as any[], unavailable: true };
    }
}
 ...
 */
async function guardContent(text, context = 'content', toast: any = null) {
